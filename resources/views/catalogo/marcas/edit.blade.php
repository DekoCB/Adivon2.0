...
<div>
        ...
        <div class="mb-4">
            <a href="{{ route('catalogo.marcas.index') }}" class="text-sm text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200 flex items-center gap-1 w-fit">
                ...
                <form action="{{ route('catalogo.marcas.update', $marca) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <img src="{{ Storage::url($marca->logo) }}" alt="{{ $marca->nombre }}"
                             class="h-20 w-20 object-contain border rounded-lg p-2 bg-gray-50 dark:bg-slate-900/60">
                    </div>
                    @endif

                    {{-- Cambiar Logo --}}
                    <div>
                        <label for="logo" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                        ...
                        <div class="flex items-center gap-4">
                            <div id="logoPreviewContainer" class="hidden">
                                <img id="logoPreview" src="" alt="Vista previa"
                                     class="h-20 w-20 object-contain rounded-lg border-2 border-blue-300 dark:border-blue-700 p-1">
                            </div>
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-xs text-gray-400 dark:text-slate-500">JPG, PNG, WEBP, SVG. Máx 2MB. Dejar vacío para mantener el logo actual.</p>
                            </div>
                        </div>
                        @error('logo')
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" id="descripcion" rows="3"
                                  ...
                                  class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">{{ old('descripcion', $marca->descripcion) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        @endphp
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Categorías</label>
                        <p class="text-xs text-gray-400 dark:text-slate-500 mb-2">Selecciona las categorías a las que pertenece esta marca</p>
                        @if($categorias->isEmpty())
                            <p class="text-sm text-gray-400 dark:text-slate-500 italic">No hay categorías activas registradas.</p>
                        @else
                            <div class="grid grid-cols-2 gap-2 border border-gray-200 dark:border-slate-700 rounded-lg p-3 max-h-48 overflow-y-auto">
                                @foreach($categorias as $categoria)
                                    <label class="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700/60 rounded p-1">
                                        <input type="checkbox"
                                               name="categorias[]"
                                               value="{{ $categoria->id }}"
