...
<div x-data="marcasPage()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-blue-500 p-4 flex justify-between items-center">
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400 uppercase font-medium">Total Marcas</p>
                    <p class="text-3xl font-bold text-gray-800 dark:text-slate-200">{{ $total }}</p>
                </div>
                <div class="bg-blue-100 dark:bg-blue-900/40 p-3 rounded-full"><i class="fas fa-tag text-blue-600 dark:text-blue-400 text-xl"></i></div>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 mb-6">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                <h2 class="text-lg font-bold text-gray-800 dark:text-slate-200">Lista de Marcas</h2>
                        ...
                        <td class="px-6 py-4">
                            @if($marca->logo)
                                <img src="{{ Storage::url($marca->logo) }}" alt="{{ $marca->nombre }}"
                                     class="w-12 h-12 object-contain rounded-lg border border-gray-200 dark:border-slate-700 p-1">
                            @else
                                <div class="w-12 h-12 rounded-lg bg-gray-100 dark:bg-slate-700 border border-gray-200 dark:border-slate-700 flex items-center justify-center">
                                    <i class="fas fa-tag text-gray-400 dark:text-slate-500"></i>
                                </div>
                            @endif
                        </td>
                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-slate-100">{{ $marca->nombre }}</td>
                        <td class="px-6 py-4 text-gray-600 dark:text-slate-400">{{ $marca->modelos_count ?? 0 }} modelos</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $marca->estado == 'activo' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
