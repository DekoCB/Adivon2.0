...
@section('content')
<div x-data="coloresPage()" x-init="init()">
        @php
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-blue-500 p-4 flex justify-between items-center">
                <div>
                    <p class="text-xs text-gray-500 dark:text-slate-400 uppercase font-medium">Total Colores</p>
                    <p class="text-3xl font-bold text-gray-800 dark:text-slate-200">{{ $total }}</p>
                </div>
                <div class="bg-blue-100 dark:bg-blue-900/40 p-3 rounded-full"><i class="fas fa-palette text-blue-600 dark:text-blue-400 text-xl"></i></div>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 mb-6">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                <h2 class="text-lg font-bold text-gray-800 dark:text-slate-200">Lista de Colores</h2>
            ...
            <x-slot:head>
                ...
                <x-th>Acciones</x-th>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/60 transition">
                        <td class="px-6 py-4">
                            @if($color->codigo_hex)
                                <div class="w-9 h-9 rounded-full border-2 border-gray-200 dark:border-slate-700 shadow-sm"
                                     style="background-color: {{ $color->codigo_hex }}"></div>
                            @else
                                <div class="w-9 h-9 rounded-full bg-gray-200 dark:bg-slate-700 border-2 border-gray-300 dark:border-slate-600 flex items-center justify-center">
                                    <i class="fas fa-question text-gray-400 dark:text-slate-500 text-xs"></i>
                                </div>
                            @endif
                        </td>
                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-slate-100">{{ $color->nombre }}</td>
                        <td class="px-6 py-4 font-mono text-sm text-gray-600 dark:text-slate-400">{{ $color->codigo_color ?? '-' }}</td>
                        <td class="px-6 py-4 font-mono text-sm text-gray-600 dark:text-slate-400">{{ $color->codigo_hex ?? '-' }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $color->estado == 'activo' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md" @click.stop>
                ...
                <form :action="formAction" method="POST" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Color (selector)</label>
                            <div class="flex items-center gap-2">
                                <input type="color" name="codigo_hex" x-model="form.codigo_hex"
                                       class="w-10 h-10 rounded border-gray-300 dark:border-slate-600 cursor-pointer">
                                <input type="text" x-model="form.codigo_hex"
                                       class="flex-1 rounded-lg border-gray-300 dark:border-slate-600 shadow-sm text-sm font-mono focus:border-blue-500 focus:ring-blue-500"
                                       placeholder="#000000" maxlength="7">
                                <input type="hidden" name="codigo_hex" :value="form.codigo_hex">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Código</label>
