...
<div>
        ...
        <div class="mb-4">
            <a href="{{ route('catalogo.modelos.index') }}" class="text-sm text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200 flex items-center gap-1 w-fit">
        ...
        <div class="max-w-2xl mx-auto">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg overflow-hidden">
                ...
                <form action="{{ route('catalogo.modelos.update', $modelo) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                    ...
                    <div>
                        <label for="marca_id" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                    ...
                    <div>
                        <label for="categoria_id" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Categoría</label>
                    ...
                    <div>
                        <label for="codigo_modelo" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Código de Modelo</label>
                        ...
                        <p class="mt-1 text-xs text-gray-400 dark:text-slate-500">Código de fábrica o referencia del modelo</p>
                        @error('codigo_modelo')
                            <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="especificaciones_tecnicas" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                        ...
                        <textarea name="especificaciones_tecnicas" id="especificaciones_tecnicas" rows="3"
                                  ...
                                  class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">{{ old('especificaciones_tecnicas', $modelo->especificaciones_tecnicas) }}</textarea>
                    </div>

                    @if($modelo->imagen_referencia)
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Imagen Actual</label>
                        <img src="{{ Storage::url($modelo->imagen_referencia) }}" alt="{{ $modelo->nombre }}"
                             class="h-24 w-24 object-cover rounded-lg border-2 border-gray-300 dark:border-slate-600">
                    </div>
                    @endif

                    <div>
                        <label for="imagen_referencia" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                            {{ $modelo->imagen_referencia ? 'Cambiar Imagen' : 'Imagen de Referencia' }}
                        </label>
                        <div class="flex items-center gap-4">
                            <div id="imagePreviewContainer" class="hidden">
                                <img id="imagePreview" src="" alt="Vista previa"
                                     class="h-24 w-24 object-cover rounded-lg border-2 border-blue-300 dark:border-blue-700">
                            </div>
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-xs text-gray-400 dark:text-slate-500">JPG, PNG, WEBP. Máx 2MB. Dejar vacío para conservar la imagen actual.</p>
                            </div>
                        </div>
