...
@section('content')
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800 dark:text-slate-200">Lista de Clientes</h2>
            @if($canCreate)
            ...
            @forelse($clientes as $cliente)
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/60">
                    <td class="px-6 py-4 text-sm">
                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-300 mr-1">{{ $cliente->tipo_documento }}</span>
                        <x-code>{{ $cliente->numero_documento }}</x-code>
                    </td>
                    <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-slate-100">{{ $cliente->nombre }}</td>
                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{{ $cliente->telefono ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{{ $cliente->ventas_count }}</td>
                    <td class="px-6 py-4">
                        <x-badge :tone="$cliente->estado === 'activo' ? 'green' : 'red'">{{ ucfirst($cliente->estado) }}</x-badge>
                    </td>
                    <td class="px-6 py-4 text-sm space-x-2">
                        @if($canEdit)
                            <a href="{{ route('clientes.edit', $cliente) }}" class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-800"><i class="fas fa-edit"></i></a>
                        @endif
                        @if($canDelete)
