...
<div>
        ...
        <div class="flex flex-wrap gap-3 mb-6">
            ...
            <span class="text-sm font-semibold text-yellow-600 dark:text-yellow-400 flex items-center gap-1.5">
        ...
        @endphp
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 mb-4 overflow-hidden"
            ...
            <div class="px-6 py-4">
                <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-2 mb-2">
                            <span class="font-mono font-bold text-blue-700 dark:text-blue-300 text-sm bg-blue-50 dark:bg-blue-900/30 px-2.5 py-0.5 rounded-lg border border-blue-200 dark:border-blue-800">
                                <i class="fas fa-file-alt mr-1 text-blue-400 text-xs"></i>{{ str_starts_with($guia, 'id:') ? 'Sin guía' : $guia }}
                            </span>
                            <span class="text-xs px-2 py-0.5 rounded-full font-semibold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300">
                            ...
                            @endif
                            @if($urgente)
                                <span class="text-xs px-2 py-0.5 rounded-full font-semibold bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400">
                                    <i class="fas fa-exclamation-triangle mr-1"></i>{{ $diasEnTransito }}d en tránsito
                                </span>
                            @endif
                            <span class="text-xs px-2.5 py-0.5 rounded-full font-semibold bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300">
                                <i class="fas fa-clock mr-1"></i>Pendiente
                            </span>
                        </div>

                        {{-- Ruta --}}
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gray-600 dark:text-slate-400">
                            <span class="flex items-center gap-1.5">
                                <i class="fas fa-warehouse text-orange-400 text-xs"></i>
                                <strong class="text-gray-800 dark:text-slate-200">{{ $primero->almacen->nombre }}</strong>
                            </span>
                            <i class="fas fa-long-arrow-alt-right text-gray-300"></i>
                            <span class="flex items-center gap-1.5">
                                <i class="fas fa-store text-green-400 text-xs"></i>
                                <strong class="text-gray-800 dark:text-slate-200">{{ $primero->almacenDestino->nombre ?? '—' }}</strong>
                            </span>
                            <span class="text-gray-300">·</span>
                        ...
                        <div class="mt-2 flex flex-wrap gap-1.5">
                            @foreach($movimientos as $mov)
                                ...
                                <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-lg border font-medium {{ $esS ? 'bg-purple-50 border-purple-200 dark:border-purple-800 text-purple-700' : 'bg-blue-50 border-blue-200 dark:border-blue-800 text-blue-700' }}">
                                    @if($esS)
                    ...
                    <div class="flex items-center gap-2 shrink-0 flex-wrap">
                        <button type="button"
                                ...
                                class="flex items-center gap-1.5 px-3.5 py-2 border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-400 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/60 transition-colors">
                ...
                <div x-show="showRechazar" x-cloak
                    ...
                    <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6 text-left">
                        <div class="flex items-center gap-3 mb-4">
                            <div class="w-10 h-10 bg-red-100 dark:bg-red-900/40 rounded-xl flex items-center justify-center">
                                <i class="fas fa-times text-red-600 dark:text-red-400"></i>
                            </div>
                            <div>
                                <h3 class="text-lg font-bold text-gray-900 dark:text-slate-100">Rechazar Recepción</h3>
                                <p class="text-sm text-gray-500 dark:text-slate-400">{{ str_starts_with($guia, 'id:') ? 'Sin guía' : $guia }}</p>
                            </div>
                        </div>
            ...
            <div x-show="detalleAbierto"
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
                        {{-- Producto header --}}
                        <div class="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/60">
                            <div class="flex items-center gap-2">
                                @if($esS)
                                    <span class="text-xs font-semibold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded">
                                        <i class="fas fa-barcode mr-1"></i>IMEI
                                    </span>
                                @else
                                    <span class="text-xs font-semibold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded">
                                        <i class="fas fa-box mr-1"></i>Accesorio
                                    </span>
                                @endif
                                <span class="font-semibold text-sm text-gray-800 dark:text-slate-200">{{ $mov->producto->nombre }}</span>
                        ...
                        <div class="px-4 py-3">
                                ...
                                @else
                                    ...
                                    <div class="flex flex-wrap gap-1.5">
                                        @foreach($mov->imeisTrasladados as $ti)
                                            <span class="font-mono text-xs bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 px-2 py-1 rounded-lg flex items-center gap-1">
                            ...
                            @else
                                <p class="text-xs text-gray-600 dark:text-slate-400">
                                    <i class="fas fa-cubes mr-1 text-gray-400 dark:text-slate-500"></i>
                                    <strong>{{ $mov->cantidad }}</strong> unidades de
                                    <strong>{{ $mov->almacen->nombre }}</strong> → <strong>{{ $mov->almacenDestino->nombre ?? '—' }}</strong>
                                    <span class="text-gray-400 dark:text-slate-500 ml-1">(stock ya descontado en origen)</span>
                                </p>
                            @endif
                        </div>
                    </div>
                    ...
                    @if($primero->transportista)
                        <p class="text-xs text-gray-500 dark:text-slate-400">
                            <i class="fas fa-truck mr-1 text-gray-400 dark:text-slate-500"></i>
                    ...
                    @if($primero->observaciones)
                        <p class="text-xs text-gray-500 dark:text-slate-400 italic">
                            <i class="fas fa-comment-alt mr-1 text-gray-400 dark:text-slate-500"></i>
                    ...
                    {{-- Urgente aviso --}}
                    @if($urgente)
                        <div class="flex items-center gap-2 text-xs text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 px-3 py-2 rounded-lg">
        ...
        @empty
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-14 text-center border border-gray-100 dark:border-slate-700">
            <div class="bg-green-50 dark:bg-green-900/30 rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-4">
                <i class="fas fa-check-circle text-4xl text-green-400"></i>
            </div>
            <p class="text-lg font-semibold text-gray-600 dark:text-slate-400">No hay traslados pendientes</p>
            <p class="text-sm text-gray-400 dark:text-slate-500 mt-1">Todos los traslados han sido confirmados</p>
