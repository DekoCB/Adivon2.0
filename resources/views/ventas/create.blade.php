...
<body class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-900 h-screen overflow-hidden font-sans antialiased">
...
<div class="flex flex-col h-screen transition-all duration-300"
    ...
    {{-- ============================
         TOPBAR (h-12)
    ============================== --}}
    <header class="h-12 bg-white dark:bg-slate-800 dark:bg-gray-800 border-b border-gray-200 dark:border-slate-700 dark:border-gray-700 flex items-center gap-2 px-3 shrink-0 shadow-sm z-20">

        {{-- Back button --}}
        <a href="{{ route('ventas.index') }}"
           class="w-8 h-8 shrink-0 flex items-center justify-center rounded-lg text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-200 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-700 dark:hover:bg-gray-700 transition">
        ...
        <div class="flex items-center gap-1 flex-1 overflow-x-auto min-w-0">
            <template x-for="(ord, idx) in ordenes" :key="ord.id">
                <button @click="cambiarOrden(idx)"
                        :class="ordenActiva === idx ? 'bg-blue-600 border-blue-600 text-white shadow-sm' : 'bg-gray-100 dark:bg-gray-700 border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 dark:hover:text-gray-200'"
            ...
            <button @click="nuevaOrden()"
                    ...
                    class="w-7 h-7 shrink-0 flex items-center justify-center rounded-lg text-gray-400 dark:text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-gray-200 dark:border-slate-700 dark:border-gray-600 transition disabled:opacity-30">
        ...
        <div class="flex items-center gap-2 shrink-0">
            <span class="text-xs text-gray-400 dark:text-slate-500 dark:text-gray-500 font-mono hidden lg:block" x-text="hora"></span>

            <button @click="toggleDarkMode()"
                    class="w-8 h-8 rounded-lg bg-gray-100 dark:bg-slate-700 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-600 dark:hover:bg-gray-600 transition"
            ...
            <span class="text-sm text-gray-600 dark:text-slate-400 dark:text-gray-300 hidden xl:block truncate max-w-30">{{ auth()->user()->name }}</span>
        </div>
    </header>
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        {{-- ═══════════════════════════════════════
             COL 1 — Products (30%)
        ═══════════════════════════════════════ --}}
        <div class="w-[30%] min-w-60 flex flex-col border-r border-gray-200 dark:border-slate-700 dark:border-gray-700 overflow-hidden bg-white dark:bg-slate-800 dark:bg-gray-800">

            {{-- Search --}}
            <div class="p-3 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700 shrink-0">
                <div class="relative">
                    <i class="fas fa-search absolute left-3 top-2.5 text-gray-400 dark:text-slate-500 text-sm pointer-events-none"></i>
                    <input type="text"
                           x-model="busqueda"
                           x-ref="searchInput"
                           @keydown.enter.prevent="buscarProductoDirecto()"
                           placeholder="Buscar... (F2)"
                           class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg pl-9 pr-8 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:text-white dark:placeholder-gray-400 transition">
                    <button x-show="busqueda" @click="busqueda=''" x-cloak
                            class="absolute right-3 top-2.5 text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300 dark:hover:text-gray-200 transition">
            ...
            {{-- Category pills --}}
            <div class="px-3 py-2 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700 overflow-x-auto shrink-0">
                <div class="flex gap-1.5 min-w-max">
                    <button @click="categoriaActiva = null"
                            :class="categoriaActiva === null ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-slate-400 dark:text-gray-300 border-gray-200 dark:border-slate-700 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-slate-600 dark:hover:bg-gray-600'"
                    ...
                    @foreach($categorias as $cat)
                        <button @click="categoriaActiva = {{ $cat->id }}"
                                :class="categoriaActiva === {{ $cat->id }} ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-slate-400 dark:text-gray-300 border-gray-200 dark:border-slate-700 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-slate-600 dark:hover:bg-gray-600'"
                                class="px-3 py-1 rounded-full text-xs font-semibold border transition whitespace-nowrap">
                            {{ $cat->nombre }}
                        </button>
                    @endforeach
            ...
            <div class="flex-1 overflow-y-auto p-3">
                ...
                <div class="grid grid-cols-3 gap-2">
                    <template x-for="producto in productosParaMostrar" :key="producto.id">
                        <div @click="agregarAlCarrito(producto)"
                             class="bg-white dark:bg-slate-800 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl overflow-hidden cursor-pointer hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-md transition-all group select-none">
                            <div class="aspect-square bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-800 relative overflow-hidden flex items-center justify-center">
                            ...
                            <div class="p-2">
                                <p class="text-[11px] text-gray-700 dark:text-slate-300 dark:text-gray-200 font-medium line-clamp-2 leading-tight mb-1" x-text="producto.nombre"></p>
                                <div class="flex items-center justify-between gap-1">
                                    ...
                                    <span x-show="orden.almacenId && producto.tipo_inventario !== 'serie'" x-cloak
                                          class="text-[9px] font-semibold text-gray-400 dark:text-slate-500 dark:text-gray-500 shrink-0"
                ...
                <template x-if="productosSinStock.length > 0">
                    <div class="mt-4">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="h-px flex-1 bg-gray-200 dark:bg-slate-700 dark:bg-gray-700"></div>
                            <span class="text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Sin stock</span>
                            <div class="h-px flex-1 bg-gray-200 dark:bg-slate-700 dark:bg-gray-700"></div>
                        </div>
                        <div class="grid grid-cols-3 gap-2 opacity-40 pointer-events-none">
                            <template x-for="producto in productosSinStock" :key="producto.id">
                                <div class="bg-white dark:bg-slate-800 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl overflow-hidden">
                                    <div class="aspect-square bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-800 relative overflow-hidden flex items-center justify-center">
                                        <template x-if="producto.imagen">
                                    ...
                                    <div class="p-2">
                                        <p class="text-[11px] text-gray-500 dark:text-slate-400 font-medium line-clamp-2 leading-tight mb-1" x-text="producto.nombre"></p>
                                        <p class="text-xs font-bold text-gray-400 dark:text-slate-500" x-text="'S/ ' + producto.precio_venta.toFixed(2)"></p>
                                    </div>
                                </div>
                            </template>
                        </div>
                ...
                <div x-show="modoMayorista && productosParaMostrar.length === 0 && productosConStock.length > 0" x-cloak
                    ...
                    <p class="text-sm font-medium text-gray-400 dark:text-slate-500">Ningún producto con stock tiene precio mayorista definido</p>
                    <p class="text-xs mt-1 text-gray-400 dark:text-slate-500">Desactiva "Mayorista" para ver el catálogo regular</p>
                </div>
                ...
                <div x-show="productosConStock.length === 0 && productosSinStock.length === 0" x-cloak
                    ...
                    <p class="text-sm font-medium text-gray-400 dark:text-slate-500">No hay productos</p>
                    <p class="text-xs mt-1 text-gray-400 dark:text-slate-500">Prueba con otra búsqueda</p>
                </div>
            </div>
        </div>
        ...
        {{-- ═══════════════════════════════════════
             COL 2 — Cart (40%)
        ═══════════════════════════════════════ --}}
        <div class="w-[40%] min-w-75 flex flex-col border-r border-gray-200 dark:border-slate-700 dark:border-gray-700 overflow-hidden">

            {{-- Cart header --}}
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-slate-700 dark:border-gray-700 bg-white dark:bg-slate-800 dark:bg-gray-800 shrink-0">
                <div class="flex items-center gap-2">
                    <h2 class="text-sm font-bold text-gray-700 dark:text-slate-300 dark:text-gray-200">Carrito</h2>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button @click="modoMayorista = !modoMayorista; recalcularPreciosCarrito()"
                            :class="modoMayorista ? 'bg-amber-500 text-white border-amber-500' : 'bg-white dark:bg-gray-700 text-gray-500 dark:text-slate-400 dark:text-gray-400 border-gray-200 dark:border-slate-700 dark:border-gray-600'"
                    ...
                    <button @click="vaciarCarrito()"
                            ...
                            class="text-xs text-gray-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition flex items-center gap-1">
            ...
            <div class="px-4 py-2.5 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700 bg-white dark:bg-slate-800 dark:bg-gray-800 shrink-0">
                <select x-model="orden.almacenId"
                        class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
            ...
            <div class="flex-1 overflow-y-auto bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-900">
                <template x-if="orden.carrito.length === 0">
                    <div class="flex flex-col items-center justify-center h-full select-none">
                        <div class="w-16 h-16 bg-gray-100 dark:bg-slate-700 dark:bg-gray-800 rounded-2xl flex items-center justify-center mb-3">
                            <i class="fas fa-shopping-cart text-2xl text-gray-300 dark:text-gray-600"></i>
                        </div>
                        <p class="text-sm font-medium text-gray-400 dark:text-slate-500">Carrito vacío</p>
                        <p class="text-xs mt-1 text-gray-400 dark:text-slate-500">Selecciona productos ←</p>
                    </div>
                </template>

                <div class="p-3 space-y-2">
                    <template x-for="(item, index) in orden.carrito" :key="index">
                        <div class="bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-xl p-3 shadow-sm hover:border-blue-200 dark:hover:border-blue-700/50 transition animate-fade-up">
                            <div class="flex items-start justify-between mb-2">
                                <div class="flex items-start gap-1.5 flex-1 pr-2 min-w-0">
                                    <span class="shrink-0 bg-gray-100 dark:bg-slate-700 dark:bg-gray-700 text-gray-400 dark:text-slate-500 dark:text-gray-500 text-[10px] font-bold rounded px-1 py-0.5 mt-0.5 leading-none" x-text="'#' + (index + 1)"></span>
                                    <p class="text-sm font-semibold text-gray-800 dark:text-slate-200 dark:text-gray-100 leading-tight line-clamp-2" x-text="item.nombre"></p>
                                </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center rounded-lg overflow-hidden bg-gray-100 dark:bg-slate-700 dark:bg-gray-700">
                                    <button @click="decrementarCantidad(index)"
                                            class="w-8 h-8 flex items-center justify-center text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 dark:hover:bg-gray-600 transition">
                                        <i class="fas fa-minus text-xs"></i>
                                    </button>
                                    <span class="w-8 text-center text-sm font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100" x-text="item.cantidad"></span>
                                ...
                                <div class="text-right">
                                    <p class="text-[11px] text-gray-400 dark:text-slate-500" x-text="'S/ ' + item.precio_unitario.toFixed(2) + ' c/u'"></p>
                                    <p class="text-base font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100" x-text="'S/ ' + (item.cantidad * item.precio_unitario).toFixed(2)"></p>
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            {{-- Notes --}}
            <div class="px-4 py-3 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700 bg-white dark:bg-slate-800 dark:bg-gray-800 shrink-0">
                <button @click="orden.showNota = !orden.showNota"
                        :class="orden.observaciones ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 hover:text-gray-600 dark:hover:text-slate-300 dark:hover:text-gray-300'"
                ...
                <div x-show="orden.showNota" x-cloak class="mt-2">
                    <textarea x-model="orden.observaciones" rows="2"
                              ...
                              class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 resize-none placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></textarea>
                </div>
            </div>
        </div>
        ...
        {{-- ═══════════════════════════════════════
             COL 3 — Checkout (30%)
        ═══════════════════════════════════════ --}}
        <div class="w-[30%] min-w-65 flex flex-col overflow-hidden bg-white dark:bg-slate-800 dark:bg-gray-800">

            @if($esVendedor)
            {{-- Tab bar: Nueva Venta / Mis Pedidos (solo Vendedor) --}}
            <div class="flex border-b border-gray-200 dark:border-slate-700 dark:border-gray-700 shrink-0">
                <button @click="tabCheckout = 'venta'"
                        :class="tabCheckout === 'venta' ? 'border-b-2 border-teal-500 text-teal-700 dark:text-teal-400 bg-white dark:bg-slate-800 dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                ...
                <button @click="tabCheckout = 'pedidos'; cargarPendientes()"
                        :class="tabCheckout === 'pedidos' ? 'border-b-2 border-teal-500 text-teal-700 dark:text-teal-400 bg-white dark:bg-slate-800 dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
            ...
            <div class="flex-1 overflow-y-auto" x-show="{{ $esVendedor ? '!esVendedor || tabCheckout === \'venta\'' : 'true' }}">
                ...
                {{-- Client --}}
                <div class="px-4 pt-4 pb-3 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
                    <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wider mb-2">Cliente</p>
                    <div class="flex gap-2">
                        <div class="flex-1 relative" @click.outside="mostrarDropdownCliente = false">
                            ...
                            <div x-show="!orden.clienteId" class="relative">
                                <i class="fas fa-user absolute left-2.5 top-2.5 text-xs text-gray-400 dark:text-slate-500 pointer-events-none"></i>
                                <input type="text"
                                       ...
                                       class="w-full border border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-700 dark:text-slate-300 dark:text-gray-200 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 rounded-lg py-2 pl-8 pr-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition placeholder-gray-400">
                            </div>
                            {{-- Dropdown --}}
                            <div x-show="mostrarDropdownCliente" x-cloak
                                 class="absolute top-full left-0 right-0 bg-white dark:bg-slate-800 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl shadow-2xl mt-1 z-40 overflow-hidden">
                                <div class="max-h-48 overflow-y-auto">
                                    <button @click="seleccionarCliente(null)"
                                            class="w-full text-left px-3 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-600 text-gray-400 dark:text-slate-500 border-b border-gray-100 dark:border-slate-700 dark:border-gray-600 transition flex items-center gap-2">
                                    ...
                                    <template x-for="c in clienteResultados" :key="c.id">
                                        <button @click="seleccionarCliente(c)"
                                                class="w-full text-left px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-600 transition border-b border-gray-100/50 dark:border-gray-600/50">
                                            <p class="text-sm text-gray-800 dark:text-slate-200 dark:text-gray-100 font-medium truncate" x-text="c.nombre"></p>
                                            <p class="text-xs text-gray-400 dark:text-slate-500 font-mono mt-0.5 flex items-center gap-1.5">
                                    ...
                                    <div x-show="clienteResultados.length === 0 && clienteQuery.length >= 2" x-cloak
                                         class="px-3 py-5 text-center text-gray-400 dark:text-slate-500 text-xs">
                        ...
                        <button @click="abrirModalCliente()" title="Nuevo cliente"
                                class="border border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:text-blue-600 hover:border-blue-400 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 rounded-lg px-3 py-2 transition">
                ...
                {{-- Comprobante --}}
                <div class="px-4 py-3 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
                    <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wider mb-2">Comprobante</p>
                    <div class="grid grid-cols-3 gap-1.5">
                        <button @click="seleccionarTipoComprobante('boleta')"
                                :class="orden.tipoComprobante === 'boleta' ? 'bg-blue-600 border-blue-600 text-white' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                        ...
                        <button @click="seleccionarTipoComprobante('factura')"
                                :class="orden.tipoComprobante === 'factura' ? 'bg-blue-600 border-blue-600 text-white' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                                class="border rounded-xl py-2 text-xs font-semibold transition flex flex-col items-center gap-1">
                            <i class="fas fa-file-invoice"></i> Factura
                        </button>
                        <button @click="seleccionarTipoComprobante('cotizacion')"
                                :class="orden.tipoComprobante === 'cotizacion' ? 'bg-amber-500 border-amber-500 text-white' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                                class="border rounded-xl py-2 text-xs font-semibold transition flex flex-col items-center gap-1">
                ...
                {{-- Guía de Remisión --}}
                <div x-show="orden.tipoComprobante !== 'cotizacion'" x-cloak class="px-4 py-3 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
                    <label class="flex items-center gap-3 cursor-pointer select-none">
                        <div class="relative shrink-0">
                            ...
                            <div :class="orden.envioProvincia ? 'translate-x-4' : 'translate-x-0.5'"
                                 class="absolute top-0.5 w-4 h-4 bg-white dark:bg-slate-800 rounded-full shadow transition-transform"></div>
                        </div>
                        <span class="text-xs text-gray-600 dark:text-slate-400 dark:text-gray-300 font-medium">Incluir guía de remisión</span>
                    </label>
                ...
                @if(!$esVendedor)
                <div class="px-4 py-3 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
                    <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wider mb-2">Formato impresión</p>
                    <div class="flex gap-2">
                        <button @click="setFormato('ticket')"
                                :class="formatoImpresion === 'ticket' ? 'bg-blue-600 border-blue-600 text-white' : 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 dark:hover:bg-gray-600'"
                        ...
                        <button @click="setFormato('a4')"
                                :class="formatoImpresion === 'a4' ? 'bg-blue-600 border-blue-600 text-white' : 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 dark:hover:bg-gray-600'"
                                class="flex-1 py-2 border rounded-xl text-xs font-semibold transition flex items-center justify-center gap-1.5">
                ...
                <div x-show="orden.tipoComprobante !== 'cotizacion'" x-cloak class="px-4 py-3 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
                    ...
                    <div class="flex items-center gap-1 mb-3 bg-gray-100 dark:bg-slate-700 dark:bg-gray-700 rounded-xl p-1">
                        <button @click="orden.condicionPago = 'contado'"
                                :class="orden.condicionPago === 'contado' ? 'bg-white dark:bg-gray-600 shadow text-blue-700 dark:text-blue-300 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 dark:hover:text-gray-200'"
                        ...
                        <button @click="orden.condicionPago = 'credito'"
                                :class="orden.condicionPago === 'credito' ? 'bg-white dark:bg-gray-600 shadow text-orange-600 dark:text-orange-400 dark:text-orange-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 dark:hover:text-gray-200'"
                                class="flex-1 py-1.5 rounded-lg text-xs font-semibold transition flex items-center justify-center gap-1">
                    ...
                    <div x-show="orden.condicionPago === 'credito'" x-cloak class="mb-3 space-y-2">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1">Nro. Cuotas</label>
                                <select x-model.number="orden.credito.numero_cuotas"
                                        class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2 py-1.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-orange-500">
                            ...
                            <div>
                                <label class="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1">Días entre cuotas</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1">Fecha primera cuota</label>
                            <input type="date" x-model="orden.credito.fecha_inicio"
                                   :min="new Date().toISOString().split('T')[0]"
                                   class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2 py-1.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-orange-500">
                        </div>
                        {{-- Preview de cuotas --}}
                        <div x-show="total > 0" class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700/50 rounded-xl p-3">
                            <p class="text-[10px] font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase mb-2">Vista previa de cuotas</p>
                            <div class="space-y-1">
                                <template x-for="i in orden.credito.numero_cuotas" :key="i">
                                    <div class="flex justify-between text-xs text-gray-600 dark:text-slate-400 dark:text-gray-300">
                    ...
                    <div class="flex items-center justify-between mb-2">
                        <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wider">Método de pago</p>
                        <button @click="agregarPago()" :disabled="orden.pagos.length >= 4"
                                class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 flex items-center gap-1 disabled:opacity-30 transition">
                    ...
                    <div x-show="orden.pagos.length === 1" class="grid grid-cols-4 gap-1.5 mb-3">
                        <button @click="seleccionarMetodoPago('efectivo')"
                                :class="orden.pagos[0].metodo === 'efectivo' ? 'bg-green-50 border-green-400 text-green-700 dark:text-green-300 dark:bg-green-900/20 dark:border-green-600 dark:text-green-400' : 'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                        ...
                        <button @click="seleccionarMetodoPago('yape')"
                                :class="orden.pagos[0].metodo === 'yape' ? 'bg-purple-50 border-purple-400 text-purple-700 dark:text-purple-300 dark:bg-purple-900/20 dark:border-purple-600 dark:text-purple-400' : 'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                                class="flex flex-col items-center gap-0.5 py-2 rounded-xl border transition text-xs font-semibold">
                            <i class="fas fa-mobile-alt text-purple-500 text-sm"></i>
                            <span>Yape</span>
                        </button>
                        <button @click="seleccionarMetodoPago('plin')"
                                :class="orden.pagos[0].metodo === 'plin' ? 'bg-teal-50 border-teal-400 text-teal-700 dark:text-teal-300 dark:bg-teal-900/20 dark:border-teal-600 dark:text-teal-400' : 'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                                class="flex flex-col items-center gap-0.5 py-2 rounded-xl border transition text-xs font-semibold">
                            <i class="fas fa-mobile-alt text-teal-500 text-sm"></i>
                            <span>Plin</span>
                        </button>
                        <button @click="seleccionarMetodoPago('transferencia')"
                                :class="orden.pagos[0].metodo === 'transferencia' ? 'bg-blue-50 border-blue-400 text-blue-700 dark:text-blue-300 dark:bg-blue-900/20 dark:border-blue-600 dark:text-blue-400' : 'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700'"
                                class="flex flex-col items-center gap-0.5 py-2 rounded-xl border transition text-xs font-semibold">
                            ...
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    <select x-model="pago.metodo"
                                            class="flex-1 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2 py-2 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500">
                                    ...
                                    <input type="number" x-model.number="pago.monto" step="0.50" min="0"
                                           ...
                                           class="w-24 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2 py-2 text-xs text-right font-mono text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500">
                                ...
                                <div x-show="pago.metodo !== 'efectivo'" x-cloak>
                                    <input type="text" x-model="pago.referencia"
                                           ...
                                           maxlength="100"
                                           class="w-full bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg px-2 py-1.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 placeholder-amber-400 dark:placeholder-amber-600 focus:ring-2 focus:ring-amber-400">
                                </div>
                            </div>
                        </template>
                    </div>
                    ...
                    <div x-show="orden.pagos.length === 1 && orden.pagos[0].metodo === 'efectivo'" x-cloak class="mt-2.5">
                        <p class="text-[10px] text-gray-400 dark:text-slate-500 mb-1.5 uppercase font-semibold tracking-wide">Pago rápido</p>
                        <div class="flex flex-wrap gap-1.5">
                            ...
                            <template x-for="amt in [10, 20, 50, 100, 200]" :key="amt">
                                <button x-show="amt >= Math.floor(total)" @click="orden.pagos[0].monto = amt"
                                        class="px-2.5 py-1 bg-gray-100 dark:bg-slate-700 dark:bg-gray-700 text-gray-600 dark:text-slate-400 dark:text-gray-300 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg text-xs font-semibold hover:bg-gray-200 dark:hover:bg-slate-600 dark:hover:bg-gray-600 transition"
                                        x-text="'S/ ' + amt"></button>
                            </template>
                        </div>
                    </div>
                        ...
                        <template x-for="(pago, pi) in orden.pagos" :key="'info'+pi">
                            <div>
                                <div x-show="['yape','plin'].includes(pago.metodo) && pagosConfig[pago.metodo]"
                                     class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 rounded-xl p-3 flex items-center gap-3">
                                    <template x-if="pagosConfig[pago.metodo]?.qr_url">
                                        <img :src="pagosConfig[pago.metodo].qr_url" class="w-16 h-16 rounded-lg bg-white dark:bg-slate-800 p-0.5 shrink-0" alt="QR">
                                    </template>
                                    <div>
                                        <p class="text-xs font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 capitalize" x-text="pago.metodo"></p>
                                        <p class="text-xs text-gray-500 dark:text-slate-400 dark:text-gray-400 mt-0.5" x-text="pagosConfig[pago.metodo]?.titular || ''"></p>
                                ...
                                <div x-show="pago.metodo === 'transferencia' && pagosConfig['transferencia']"
                                     class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 rounded-xl p-3 text-xs space-y-0.5">
                                    <p class="font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 mb-1">Transferencia Bancaria</p>
                                    <template x-if="pagosConfig['transferencia']?.banco">
                                        <p class="text-gray-500 dark:text-slate-400 dark:text-gray-400">Banco: <span class="text-gray-700 dark:text-slate-300 dark:text-gray-200 font-semibold" x-text="pagosConfig['transferencia'].banco"></span></p>
                                    </template>
                                    <template x-if="pagosConfig['transferencia']?.numero">
                                        <p class="text-gray-500 dark:text-slate-400 dark:text-gray-400">N° Cta: <span class="text-gray-700 dark:text-slate-300 dark:text-gray-200 font-mono font-semibold" x-text="pagosConfig['transferencia'].numero"></span></p>
                                    </template>
                                    <template x-if="pagosConfig['transferencia']?.titular">
                                        <p class="text-gray-500 dark:text-slate-400 dark:text-gray-400">A nombre de: <span class="text-gray-700 dark:text-slate-300 dark:text-gray-200 font-semibold" x-text="pagosConfig['transferencia'].titular"></span></p>
                                    </template>
                                </div>
                            </div>
                        </template>
            ...
            <div x-show="tabCheckout === 'pedidos'" x-cloak class="flex-1 overflow-y-auto flex flex-col">
                ...
                {{-- Header con refresh --}}
                <div class="px-4 py-3 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700 flex items-center justify-between shrink-0">
                    <div>
                        <p class="text-xs font-bold text-gray-700 dark:text-slate-300 dark:text-gray-200 uppercase tracking-wider">Ventas pendientes de cobro</p>
                        <p class="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">Se actualiza automáticamente cada 30s</p>
                    </div>
                    <button @click="cargarPendientes()" :disabled="cargandoPendientes"
                            class="w-7 h-7 rounded-lg bg-gray-100 dark:bg-slate-700 dark:bg-gray-700 text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:bg-teal-100 hover:text-teal-600 dark:hover:bg-teal-900/30 dark:hover:text-teal-400 flex items-center justify-center transition disabled:opacity-40"
                            title="Actualizar">
                ...
                <div class="flex-1 overflow-y-auto p-3 space-y-3">
                    ...
                    <template x-if="pendientes.length === 0 && !cargandoPendientes">
                        <div class="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p class="text-sm font-medium text-gray-400 dark:text-slate-500 dark:text-gray-500">Sin pedidos pendientes</p>
                    ...
                    <template x-for="p in pendientes" :key="p.id">
                        <div class="bg-white dark:bg-slate-800 dark:bg-gray-800 border-2 border-amber-200 dark:border-amber-700 rounded-2xl overflow-hidden shadow-sm">

                            {{-- Header tarjeta --}}
                            <div class="bg-amber-50 dark:bg-amber-900/20 px-3 py-2.5 flex items-start justify-between gap-2">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span class="font-mono font-bold text-sm text-gray-800 dark:text-slate-200 dark:text-gray-100" x-text="p.codigo"></span>
                                    ...
                                    <p class="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5" x-text="'hace ' + p.hace + ' · ' + p.hora"></p>
                                ...
                                <div class="text-right shrink-0">
                                    <div class="text-base font-bold text-gray-900 dark:text-slate-100 dark:text-gray-100" x-text="'S/ ' + parseFloat(p.total).toFixed(2)"></div>
                                    <div class="text-[10px] text-gray-400 dark:text-slate-500" x-text="p.items_count + ' ítem(s)'"></div>
                                </div>
                            </div>
                            ...
                            <div x-data="{ abierto: false }">
                                <button @click="abierto = !abierto"
                                        class="w-full px-3 py-2 text-left text-[10px] text-gray-500 dark:text-slate-400 dark:text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700/50 flex items-center justify-between transition">
                                    <span>Ver productos</span>
                                    <i class="fas fa-chevron-down transition-transform" :class="abierto ? 'rotate-180' : ''"></i>
                                </button>
                                <div x-show="abierto" x-cloak class="border-t border-gray-100 dark:border-slate-700 dark:border-gray-700">
                                    <template x-for="(d, di) in p.detalles" :key="d.id ?? di">
                                        <div class="px-3 py-1.5 flex items-center justify-between gap-2 text-xs border-b border-gray-50 dark:border-gray-700/50 last:border-0">
                                            <div class="flex-1 min-w-0">
                                                <p class="font-medium text-gray-800 dark:text-slate-200 dark:text-gray-200 truncate" x-text="d.producto"></p>
                                                <p class="text-[10px] text-gray-400 dark:text-slate-500" x-text="'× ' + d.cantidad"></p>
                                            </div>
                                            <span class="font-semibold text-gray-700 dark:text-slate-300 dark:text-gray-300 whitespace-nowrap font-mono text-[11px]"
                            ...
                            {{-- Footer: ver detalle --}}
                            <div class="px-3 py-2.5 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700/30 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700">
                                <a :href="'/ventas/' + p.id"
                ...
                <div class="px-4 py-2 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700 shrink-0 text-center">
                    <p class="text-[10px] text-gray-400 dark:text-slate-500 dark:text-gray-500">
            ...
            <div class="border-t border-gray-200 dark:border-slate-700 dark:border-gray-700 bg-white dark:bg-slate-800 dark:bg-gray-800 px-4 py-3 shrink-0"
                 x-show="{{ $esVendedor ? 'tabCheckout === \'venta\'' : 'true' }}">
                <div class="space-y-1 mb-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500 dark:text-slate-400 dark:text-gray-400">Subtotal</span>
                        <span class="text-gray-700 dark:text-slate-300 dark:text-gray-300 font-medium" x-text="'S/ ' + subtotal.toFixed(2)"></span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500 dark:text-slate-400 dark:text-gray-400">IGV (18%)</span>
                        <span class="text-gray-700 dark:text-slate-300 dark:text-gray-300 font-medium" x-text="'S/ ' + igv.toFixed(2)"></span>
                    </div>
                    <div class="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-slate-700 dark:border-gray-700">
                        <span class="text-base font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100">TOTAL</span>
                ...
                <template x-if="!cajaAbierta && orden.tipoComprobante !== 'cotizacion' && orden.condicionPago !== 'credito'">
                    <div class="flex items-center gap-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl px-3 py-2 mb-1">
                ...
                <button @click="procesarPago()"
                        ...
                        :class="{ 'bg-amber-500 hover:bg-amber-600 shadow-amber-200 dark:shadow-amber-900/30': orden.tipoComprobante === 'cotizacion', 'bg-orange-600 hover:bg-orange-700 shadow-orange-200 dark:shadow-orange-900/30': orden.condicionPago === 'credito' && orden.tipoComprobante !== 'cotizacion', 'bg-blue-600 hover:bg-blue-700 shadow-blue-200 dark:shadow-blue-900/30': orden.condicionPago !== 'credito' && orden.tipoComprobante !== 'cotizacion', }"
...
<div class="fixed top-4 right-4 z-100 space-y-2 pointer-events-none" x-cloak>
    <template x-for="t in toasts" :key="t.id">
        <div class="flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl text-sm font-medium animate-slide-in-right max-w-xs"
             :class="{ 'bg-green-600 text-white': t.tipo === 'success', 'bg-red-600 text-white': t.tipo === 'error', 'bg-blue-600 text-white': t.tipo === 'info', 'bg-amber-500 text-white': t.tipo === 'warning', }">
            <i class="fas shrink-0"
               :class="{ 'fa-check-circle': t.tipo === 'success', 'fa-exclamation-circle': t.tipo === 'error', 'fa-info-circle': t.tipo === 'info', 'fa-exclamation-triangle':t.tipo === 'warning', }"></i>
...
<div x-show="showPago" x-cloak class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-2xl w-full max-w-sm shadow-2xl animate-fade-up">

        <div class="p-5 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
            <h3 class="text-lg font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100"
                x-text="orden.tipoComprobante === 'cotizacion' ? 'Guardar Cotización' : 'Confirmar Venta'"></h3>
            <p class="text-sm text-gray-500 dark:text-slate-400 dark:text-gray-400 mt-0.5"
        ...
        <div class="p-5 space-y-4">
            {{-- Summary --}}
            <div class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700/50 rounded-xl p-4 space-y-2">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500 dark:text-slate-400 dark:text-gray-400">Comprobante</span>
                    <span class="font-semibold text-gray-700 dark:text-slate-300 dark:text-gray-200 capitalize" x-text="orden.tipoComprobante"></span>
                </div>
                <div x-show="orden.tipoComprobante !== 'cotizacion' && !esVendedor" class="flex justify-between text-sm">
                    <span class="text-gray-500 dark:text-slate-400 dark:text-gray-400">Formato</span>
                    <span class="font-semibold text-gray-700 dark:text-slate-300 dark:text-gray-200" x-text="formatoImpresion === 'ticket' ? 'Ticket 80mm' : 'A4'"></span>
                </div>
                <div x-show="orden.tipoComprobante !== 'cotizacion' && orden.condicionPago !== 'credito' && orden.condicionPago !== 'pendiente_cobro'" class="flex justify-between text-sm">
                    <span class="text-gray-500 dark:text-slate-400 dark:text-gray-400">Método</span>
                    <span class="font-semibold text-gray-700 dark:text-slate-300 dark:text-gray-200 capitalize"
                ...
                <div x-show="orden.condicionPago === 'credito'" class="flex justify-between text-sm">
                    <span class="text-gray-500 dark:text-slate-400 dark:text-gray-400">Condición</span>
                ...
                <div class="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-slate-700 dark:border-gray-600">
                    <span class="text-base font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100">Total</span>
                ...
                <div x-show="vuelto > 0 && orden.tipoComprobante !== 'cotizacion'" x-cloak class="flex justify-between text-sm">
                    <span class="text-green-600 dark:text-green-400 font-semibold">Vuelto a entregar</span>
                    <span class="font-bold text-green-600 dark:text-green-400" x-text="'S/ ' + vuelto.toFixed(2)"></span>
                </div>
            </div>
        ...
        <div class="flex gap-3 p-5 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700">
            <button @click="showPago = false"
                    class="flex-1 border border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700 rounded-xl py-3 font-semibold text-sm transition">
                Cancelar
            </button>
...
<div x-show="showModalCliente" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
    ...
    <div class="relative bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-2xl w-full max-w-md shadow-2xl animate-fade-up max-h-[90vh] overflow-y-auto">
        <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700 sticky top-0 bg-white dark:bg-slate-800 dark:bg-gray-800 z-10">
            <h3 class="text-base sm:text-lg font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 flex items-center gap-2">
                <i class="fas fa-user-plus text-blue-500"></i> Nuevo Cliente
            </h3>
            <p class="text-xs text-gray-500 dark:text-slate-400 dark:text-gray-400 mt-0.5">Consulta DNI/RUC o ingresa manualmente</p>
        </div>
        ...
        <div class="p-4 sm:p-5 space-y-4">
            ...
            <div class="grid grid-cols-5 gap-2">
                <div class="col-span-2">
                    <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">Tipo</label>
                    <select x-model="nuevoCliente.tipo_documento"
                            class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2.5 py-2.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 truncate">
                ...
                <div class="col-span-3">
                    <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">N° Documento</label>
                    <div class="flex gap-1.5">
                        <input type="text" x-model="nuevoCliente.numero_documento"
                               :maxlength="nuevoCliente.tipo_documento === 'RUC' ? 11 : 8"
                               class="flex-1 min-w-0 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2.5 py-2.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 font-mono"
            ...
            <!-- Nombre / Razón social -->
            <div>
                <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">Nombre / Razón social</label>
                <input type="text" x-model="nuevoCliente.nombre"
                       class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2.5 py-2.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500"
                       placeholder="Ingrese nombre o razón social">
            </div>

            <!-- Teléfono -->
            <div>
                <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">Teléfono</label>
            ...
            <!-- Dirección -->
            <div>
                <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">Dirección</label>
            ...
            <!-- Departamento -->
            <div>
                <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">Departamento</label>
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">Provincia</label>
                    <input type="text" x-model="nuevoCliente.provincia"
                           class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2.5 py-2.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500"
                           placeholder="Provincia">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1">Distrito</label>
        ...
        <!-- Botones -->
        <div class="flex gap-2 p-4 sm:p-5 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700 sticky bottom-0 bg-white dark:bg-slate-800 dark:bg-gray-800">
            <button @click="showModalCliente = false"
                    class="flex-1 border border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700 rounded-xl py-2.5 font-semibold text-xs transition">
                Cancelar
            </button>
...
<div x-show="showModalEditarCliente" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
    ...
    <div class="relative bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-2xl w-full max-w-md shadow-2xl animate-fade-up max-h-[90vh] overflow-y-auto">
        <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700 sticky top-0 bg-white dark:bg-slate-800 dark:bg-gray-800 z-10">
            ...
            <p class="text-xs text-gray-500 dark:text-slate-400 dark:text-gray-400 mt-0.5" x-text="editandoCliente.nombre"></p>
        </div>
            ...
            <div class="grid grid-cols-5 gap-2">
                <div class="col-span-2">
                    ...
                    <select x-model="editandoCliente.tipo_documento"
                            class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2.5 py-2.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500">
                        <option value="DNI">DNI</option>
                        <option value="RUC">RUC</option>
                        <option value="CE">Carnet Ext.</option>
                        <option value="PASAPORTE">Pasaporte</option>
                ...
                <div class="col-span-3">
                    ...
                    <input type="text" x-model="editandoCliente.numero_documento"
                           ...
                           class="w-full bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-2.5 py-2.5 text-xs text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 font-mono">
                </div>
            </div>
...
<div x-show="mostrarModalVariante" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl animate-fade-up">
        <div class="p-5 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
            <h3 class="text-lg font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 flex items-center gap-2">
                <i class="fas fa-swatchbook text-indigo-500"></i>
                Seleccionar Variante
            </h3>
            <p class="text-sm text-gray-500 dark:text-slate-400 dark:text-gray-400 mt-0.5" x-text="productoActual?.nombre"></p>
        </div>
        <div class="p-5 max-h-[60vh] overflow-y-auto">
            <div class="grid grid-cols-2 gap-2.5">
                <template x-for="v in variantesParaMostrar()" :key="v.id">
                    <button @click="seleccionarVariante(v)"
                            class="text-left border rounded-xl p-3 transition group border-gray-200 dark:border-slate-700 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20">
                        <div class="flex items-center gap-2 mb-1.5">
                            ...
                            <span class="text-sm font-semibold text-gray-800 dark:text-slate-200 dark:text-gray-100 truncate" x-text="v.nombre_completo"></span>
                        </div>
            ...
            <div x-show="variantesConStock().length === 0" class="py-8 text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-slate-400 dark:text-gray-400">No hay variantes con stock en este punto de venta</p>
            </div>
            <div x-show="variantesConStock().length > 0 && variantesParaMostrar().length === 0" class="py-8 text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-slate-400 dark:text-gray-400">Ninguna variante con stock tiene precio mayorista definido</p>
            </div>
        </div>
        <div class="p-4 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700">
            <button @click="mostrarModalVariante = false"
                    class="w-full border border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700 rounded-xl py-2.5 font-semibold text-sm transition">
                Cancelar
            </button>
        </div>
    </div>
...
<div x-show="mostrarModalIMEI" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl animate-fade-up">
        <div class="p-5 border-b border-gray-100 dark:border-slate-700 dark:border-gray-700">
            ...
            <p class="text-sm text-gray-500 dark:text-slate-400 dark:text-gray-400 mt-0.5" x-text="(productoActual?.nombre || '') + (varianteActual?.nombre_completo ? ' — ' + varianteActual.nombre_completo : '')"></p>
        </div>
        <div class="p-5 space-y-4">
            {{-- Manual entry --}}
            <div>
                <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-1.5">Ingresar IMEI/Serie manualmente</label>
                <div class="flex gap-2">
                    <input type="text" x-model="imeiActual"
                           @keydown.enter.prevent="agregarIMEIManual()"
                           placeholder="15 dígitos..."
                           maxlength="15"
                           class="flex-1 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-lg px-3 py-2.5 text-sm font-mono text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-purple-500 placeholder-gray-400">
            ...
            <div x-show="imeisTemp.length > 0" x-cloak>
                <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-2">Seleccionados (<span x-text="imeisTemp.length"></span>)</p>
                <div class="flex flex-wrap gap-1.5">
            ...
            <div>
                <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 mb-2">
                ...
                <div x-show="!cargandoImeis && imeisDisponibles.length === 0" x-cloak
                     class="text-xs text-gray-400 dark:text-slate-500 text-center py-4 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-700 rounded-xl">
        ...
        <div class="flex gap-2 p-5 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700">
            <button @click="_limpiarModalIMEI()"
                    class="border border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-700 rounded-xl py-2.5 px-4 font-semibold text-sm transition">
                Cancelar
            </button>
...
<div x-show="showModalGuia" x-cloak
    ...
    <div class="relative w-full max-w-3xl bg-white dark:bg-slate-800 dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden"
         x-transition:enter="transition ease-out duration-200"
        ...
        <div class="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
            ...
            <div>
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <h3 class="text-sm font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 uppercase tracking-wide">Cliente / Destinatario</h3>
                </div>
                <div class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-xl p-4">
                    <div x-show="orden.clienteId" class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div class="md:col-span-2">
                            <p class="text-xs text-gray-400 dark:text-slate-500 mb-1 font-medium uppercase tracking-wide">Nombre / Razón Social</p>
                            <p class="text-sm font-semibold text-gray-800 dark:text-slate-200 dark:text-gray-100" x-text="orden.clienteNombre || '—'"></p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 dark:text-slate-500 mb-1 font-medium uppercase tracking-wide">Documento</p>
                            <p class="text-sm font-mono text-gray-700 dark:text-slate-300 dark:text-gray-200">
            ...
            <div>
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <h3 class="text-sm font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 uppercase tracking-wide">Datos de Traslado</h3>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1.5">
                            Motivo del Traslado <span class="text-red-500">*</span>
                        </label>
                        <select x-model="orden.guia.motivo_traslado"
                                class="w-full bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <label :class="orden.guia.modalidad === 'privado' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 dark:text-blue-300' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:border-blue-300'"
                            ...
                            <label :class="orden.guia.modalidad === 'publico' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 dark:text-blue-300' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:border-blue-300'"
                                   class="flex items-center gap-2 border rounded-xl px-3 py-2.5 cursor-pointer transition">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1.5">Nro. Bultos</label>
            ...
            <div x-show="orden.guia.modalidad === 'privado'">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <h3 class="text-sm font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 uppercase tracking-wide">
                        Datos del Conductor
                    ...
                    <button type="button"
                            ...
                            class="ml-auto text-[10px] text-gray-400 dark:text-slate-500 hover:text-red-500 transition flex items-center gap-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <div class="relative flex gap-2">
                            <input type="text" x-model="orden.guia.conductor_dni"
                                   ...
                                   class="flex-1 bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition font-mono">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" x-model="orden.guia.conductor_nombre"
                               ...
                               class="w-full bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                    </div>
                    {{-- Placa --}}
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1.5">
                        ...
                        <input type="text" x-model="orden.guia.placa_vehiculo"
                               ...
                               class="w-full bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition font-mono tracking-widest">
                    </div>
                    {{-- Licencia --}}
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1.5">
                        ...
                        <input type="text" x-model="orden.guia.conductor_licencia"
                               ...
                               class="w-full bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition font-mono">
                    </div>
                </div>
            </div>
            ...
            <div x-show="orden.guia.modalidad === 'publico'">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1.5">Tipo Documento</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1.5">Nro. Documento</label>
                        <div class="flex gap-2">
                            <input type="text" x-model="orden.guia.transportista_doc" placeholder="Nro. documento"
                                   ...
                                   class="flex-1 bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition font-mono">
                ...
                <div x-show="transportistaCamposExtra.length" x-cloak
                     class="mt-2 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-800/60 border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-xl px-3 py-2.5">
                    <p class="text-[10px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide mb-1.5">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-1 text-xs">
                        <template x-for="[label, valor] in transportistaCamposExtra" :key="label">
                            <div class="truncate"><span class="text-gray-400 dark:text-slate-500" x-text="label + ':'"></span> <span class="text-gray-700 dark:text-slate-300 dark:text-gray-200 font-medium" x-text="valor"></span></div>
                        </template>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide mb-1.5">Ubigeo</label>
                        <input type="text" x-model="orden.guia.ubigeo_partida" placeholder="Ej: 150101" maxlength="6"
                               class="w-full bg-white dark:bg-slate-800 dark:bg-gray-800 border border-gray-200 dark:border-slate-700 dark:border-gray-600 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-slate-300 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition font-mono">
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <h3 class="text-sm font-bold text-gray-800 dark:text-slate-200 dark:text-gray-100 uppercase tracking-wide">Detalle de Traslado</h3>
                    <span class="text-xs text-gray-400 dark:text-slate-500 font-normal">(cargado automáticamente del carrito)</span>
                </div>
                <div class="border border-gray-200 dark:border-slate-700 dark:border-gray-700 rounded-xl overflow-hidden">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-800">
                            <tr>
                                <th class="px-4 py-2.5 text-left text-xs font-bold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide">Producto</th>
                                <th class="px-4 py-2.5 text-center text-xs font-bold text-gray-500 dark:text-slate-400 dark:text-gray-400 uppercase tracking-wide w-20">Cant.</th>
                            </tr>
                        </thead>
                        <tbody>
                            <template x-for="(item, idx) in orden.carrito" :key="idx">
                                <tr class="border-t border-gray-100 dark:border-slate-700 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-slate-700/60 dark:hover:bg-gray-800/50">
                                    <td class="px-4 py-2.5 text-gray-700 dark:text-slate-300 dark:text-gray-200 text-xs" x-text="item.nombre"></td>
                                    <td class="px-4 py-2.5 text-center text-gray-700 dark:text-slate-300 dark:text-gray-200 text-xs font-bold" x-text="item.cantidad"></td>
                                </tr>
                            </template>
                            <template x-if="orden.carrito.length === 0">
                                <tr>
                                    <td colspan="2" class="px-4 py-4 text-center text-xs text-gray-400 dark:text-slate-500">
                                        <i class="fas fa-shopping-cart mr-1"></i> No hay productos en el carrito
                                    </td>
                                </tr>
                            </template>
                        </tbody>
                        <tfoot class="bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-800 border-t border-gray-200 dark:border-slate-700 dark:border-gray-700">
                            <tr>
                                <td class="px-4 py-2 text-xs font-semibold text-gray-500 dark:text-slate-400 dark:text-gray-400">Total</td>
                                <td class="px-4 py-2 text-center text-xs font-bold text-gray-700 dark:text-slate-300 dark:text-gray-200"
        ...
        {{-- Footer con botones --}}
        <div class="px-6 py-4 bg-gray-50 dark:bg-slate-900/60 dark:bg-gray-800/50 border-t border-gray-100 dark:border-slate-700 dark:border-gray-700 flex gap-3">
            <button @click="cerrarModalGuia()" type="button"
                    class="flex-1 border border-gray-200 dark:border-slate-700 dark:border-gray-600 text-gray-600 dark:text-slate-400 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 dark:hover:bg-gray-700 rounded-xl py-3 font-semibold text-sm transition">
...
<div x-data="{ show: true }" x-show="show" x-cloak
    ...
    <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden"
        ...
        <div class="p-6 text-center">
            <div class="text-gray-500 dark:text-slate-400 text-sm mb-1">Total cobrado</div>
            <div class="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-5">S/ {{ $ve['total'] }}</div>
