...
<div>
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400 mb-4">
        ...
        <span class="text-gray-800 dark:text-slate-200 font-medium truncate">{{ $producto->nombre }}</span>
    </nav>

    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">{{ $producto->nombre }}</h1>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Gestión de precios y márgenes</p>
        </div>
        ...
        <div class="space-y-5"
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
                ...
                <div class="p-5 space-y-3">
                    ...
                    <div class="flex items-center justify-between text-sm border-t border-gray-100 dark:border-slate-700 pt-3 mt-3">
                        ...
                        <span class="font-bold text-emerald-700 dark:text-emerald-300 text-base">
                            @if($precioVentaMin == $precioVentaMax)
                                S/ {{ number_format($precioVentaMin, 2) }}
                            @else
                                S/ {{ number_format($precioVentaMin, 2) }} – S/ {{ number_format($precioVentaMax, 2) }}
                            @endif
                        </span>
                    </div>
                    @if($precioVentaMin == $precioVentaMax)
                    <div class="flex items-center justify-between text-xs text-gray-400 dark:text-slate-500 mt-1">
                        <span>Base: S/ {{ number_format($precioVentaMin / 1.18, 2) }}</span>
                        <span>IGV: S/ {{ number_format($precioVentaMin - ($precioVentaMin / 1.18), 2) }}</span>
                    </div>
                    @else
                    <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">Varía según la capacidad — ver detalle a la derecha.</p>
                    @endif
                    @elseif($producto->precio_venta > 0)
                    <div class="flex items-center justify-between text-sm border-t border-gray-100 dark:border-slate-700 pt-3 mt-3">
                        <span class="text-gray-500 dark:text-slate-400 font-semibold">Precio de Venta</span>
                        <span class="font-bold text-emerald-700 dark:text-emerald-300 text-base">S/ {{ number_format($producto->precio_venta, 2) }}</span>
                    </div>
                    <div class="flex items-center justify-between text-xs text-gray-400 dark:text-slate-500 mt-1">
                ...
                <form method="POST" action="{{ route('precios.store', $producto) }}" class="p-5 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wide mb-1">
                            Capacidad <span class="text-gray-400 dark:text-slate-500 normal-case font-normal">(el precio aplica a todos los colores de esa capacidad)</span>
                        </label>
                        <select name="variante_id" x-model="varianteId"
                                @change="cambiarVariante()"
                                class="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wide mb-1">
                            Proveedor <span class="text-gray-400 dark:text-slate-500 normal-case font-normal">(opcional)</span>
                        </label>
                        ...
                        <div class="relative" @click.away="abiertoDropdown = false">
                            <input type="text"
                                   ...
                                   class="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 pr-8 text-sm focus:ring-2 focus:ring-emerald-500">
                            ...
                            <div x-show="proveedorId && !abiertoDropdown" class="mt-1.5">
                                <span class="inline-flex items-center gap-1 text-xs text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 px-2 py-0.5 rounded-full">
                    ...
                    {{-- Precio compra --}}
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wide mb-1">Precio Compra (S/)</label>
                        ...
                        <div x-show="ultimaCompra" class="mt-1.5 flex items-start gap-1.5 text-xs text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 px-2.5 py-1.5 rounded-lg">
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                            <button type="button"
                                    ...
                                    :class="modoCalculo==='margen' ? 'bg-emerald-600 text-white' : 'bg-white dark:bg-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-600'"
                            ...
                            <button type="button"
                                    ...
                                    :class="modoCalculo==='precio' ? 'bg-emerald-600 text-white' : 'bg-white dark:bg-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-600'"
                    ...
                    {{-- Margen % --}}
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wide mb-1">Margen %</label>
                    ...
                    <div>
                        ...
                        <input type="number" x-model="precioVenta"
                               ...
                               :class="modoCalculo==='margen' ? 'bg-gray-50 text-emerald-700 dark:text-emerald-300 font-semibold cursor-not-allowed' : 'text-emerald-700 font-semibold'"
                    ...
                    <template x-if="resultado">
                        <div class="bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 space-y-2">
                            <div class="flex justify-between items-center text-sm">
                                <span class="font-semibold text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5">
                                    <i class="fas fa-tag text-emerald-500 text-xs"></i> Precio de Venta
                                </span>
                                <span class="font-bold text-emerald-800 dark:text-emerald-300 text-lg" x-text="'S/ ' + resultado.precio_final.toFixed(2)"></span>
                            </div>
                            <div class="flex justify-between text-xs text-gray-500 dark:text-slate-400 border-t border-emerald-200 dark:border-emerald-800 pt-2">
                                <span>Margen</span>
                    ...
                    <div class="border-t border-dashed border-amber-200 dark:border-amber-800 pt-4 mt-2 space-y-3">
                        <p class="text-xs font-bold text-amber-700 dark:text-amber-300 uppercase tracking-wide flex items-center gap-1.5">
                            <i class="fas fa-tags"></i> Precio Mayorista
                            <span class="text-gray-400 dark:text-slate-500 normal-case font-normal">(opcional)</span>
                        </p>
                        ...
                        <div>
                            ...
                            <div class="flex rounded-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                                <button type="button"
                                        ...
                                        :class="modoCalculoMayor==='margen' ? 'bg-amber-500 text-white' : 'bg-white dark:bg-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-600'"
                                        class="flex-1 py-2 text-xs font-semibold transition-colors border-r border-gray-200 dark:border-slate-700">
                                    <i class="fas fa-percentage mr-1"></i> Margen %
                                </button>
                                <button type="button"
                                        @click="modoCalculoMayor='precio'"
                                        :class="modoCalculoMayor==='precio' ? 'bg-amber-500 text-white' : 'bg-white dark:bg-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-600'"
                                        class="flex-1 py-2 text-xs font-semibold transition-colors">
                        ...
                        {{-- Margen mayorista --}}
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wide mb-1">Margen Mayorista %</label>
                            <input type="number" x-model="margenMayor"
                                   @input="if(modoCalculoMayor==='margen') calcularMayorista()"
                                   :readonly="modoCalculoMayor==='precio'"
                                   :class="modoCalculoMayor==='precio' ? 'bg-gray-50 text-gray-500 dark:text-slate-400 cursor-not-allowed' : ''"
                                   step="0.1" min="0" max="1000"
                                   class="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400">
                            <p x-show="modoCalculoMayor==='precio'" class="text-xs text-gray-400 dark:text-slate-500 mt-1">Calculado según el precio ingresado</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" x-model="precioMayorista"
                                   ...
                                   :class="modoCalculoMayor==='margen' ? 'bg-gray-50 text-amber-700 dark:text-amber-300 font-semibold cursor-not-allowed' : 'text-amber-700 font-semibold'"
                                   step="0.01" min="0.01" placeholder="0.00"
                                   class="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400">
                            <p x-show="modoCalculoMayor==='margen'" class="text-xs text-gray-400 dark:text-slate-500 mt-1">Calculado según el margen</p>
                        </div>
                        ...
                        <template x-if="resultadoMayor">
                            <div class="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-xl p-4 space-y-2">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-slate-400">Margen mayorista</span>
                                    <span class="font-semibold text-amber-700 dark:text-amber-300" x-text="(parseFloat(margenMayor)||0).toFixed(1) + '%'"></span>
                                </div>
                                <div class="flex justify-between text-sm border-t border-amber-200 dark:border-amber-800 pt-2">
                                    <span class="font-semibold text-gray-700 dark:text-slate-300">Precio mayorista</span>
                                    <span class="font-bold text-amber-700 dark:text-amber-300 text-lg" x-text="'S/ ' + resultadoMayor.precio_final.toFixed(2)"></span>
                                </div>
                                <div class="flex justify-between text-xs text-gray-400 dark:text-slate-500 pt-1" x-show="resultado">
                    ...
                    {{-- Observaciones --}}
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wide mb-1">Observaciones</label>
                        <textarea name="observaciones" rows="2" placeholder="Motivo del precio, notas..."
                                  class="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 resize-none"></textarea>
                    </div>
                    ...
                    <label class="flex items-start gap-3 cursor-pointer p-3 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-emerald-800 dark:text-emerald-300">Replicar a todas las tiendas</p>
                    ...
                    <p x-show="!resultado" x-cloak class="text-xs text-center text-gray-400 dark:text-slate-500">
        ...
        <div class="xl:col-span-2 space-y-5">
                    ...
                    @foreach($porCapacidadGlobal as $cap => $vars)
                        ...
                        <div class="bg-white dark:bg-slate-800 rounded-xl border {{ $esActual ? 'border-blue-400 ring-2 ring-blue-100' : 'border-gray-100' }} shadow-sm p-4 cursor-pointer hover:border-blue-300 dark:hover:border-blue-600 transition-all"
                            ...
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <span class="text-xs text-gray-400 dark:text-slate-500">{{ $vars->count() }} color{{ $vars->count() > 1 ? 'es' : '' }}</span>
                            </div>
                            @if($pCap)
                                <div class="space-y-1.5">
                                    <div class="flex justify-between text-xs">
                                        <span class="text-gray-500 dark:text-slate-400">Compra</span>
                                        <span class="font-medium text-gray-700 dark:text-slate-300">{{ $pCap->precio_compra ? 'S/ ' . number_format($pCap->precio_compra, 2) : '—' }}</span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        <span class="text-xs text-gray-500 dark:text-slate-400">P. Venta</span>
                                        <span class="text-base font-bold text-blue-700 dark:text-blue-300">S/ {{ number_format($pCap->precio, 2) }}</span>
                                    </div>
                                    <div class="flex justify-between text-xs border-t border-gray-100 dark:border-slate-700 pt-1.5 mt-1.5">
                                    ...
                                    @if($pMay)
                                        <div class="flex justify-between items-center border-t border-amber-100 dark:border-amber-800 pt-1.5 mt-1.5 bg-amber-50 dark:bg-amber-900/30 -mx-1 px-1 rounded">
                                            ...
                                            <div class="text-right">
                                                <span class="text-sm font-bold text-amber-700 dark:text-amber-300">S/ {{ number_format($pMay->precio, 2) }}</span>
                                                @if($pMay->margen)
                            ...
                            @else
                                <div class="flex items-center justify-center py-3">
                                    <span class="inline-flex items-center gap-1.5 text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 px-3 py-1.5 rounded-full">
            ...
            @else
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="bg-white dark:bg-slate-800 rounded-xl border border-blue-200 dark:border-blue-800 shadow-sm p-4 text-center">
                        <p class="text-xs text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-1 font-semibold">Precio de Venta</p>
                        <p class="text-xl font-bold text-blue-700 dark:text-blue-300">
                            {{ $precioGlobal ? 'S/ ' . number_format($precioGlobal->precio, 2) : '—' }}
                        </p>
                        @if($precioGlobal)
                            <p class="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">Base: S/ {{ number_format($precioGlobal->precio / 1.18, 2) }} + IGV</p>
                        @endif
                    </div>
                    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-4 text-center">
                        <p class="text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Precio Compra</p>
                        <p class="text-xl font-bold text-gray-700 dark:text-slate-300">
                            {{ $precioGlobal ? 'S/ ' . number_format($precioGlobal->precio_compra, 2) : '—' }}
                        </p>
                    </div>
                    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-4 text-center">
                        <p class="text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Margen</p>
                        <p class="text-xl font-bold {{ ($precioGlobal?->margen ?? 0) >= 20 ? 'text-green-700' : 'text-yellow-600' }}">
                            {{ $precioGlobal ? $precioGlobal->margen . '%' : '—' }}
                        </p>
                    </div>
                    <div class="bg-white dark:bg-slate-800 rounded-xl border {{ $precioMayoristaGlobal ? 'border-amber-200 bg-amber-50' : 'border-gray-100' }} shadow-sm p-4 text-center">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100 dark:divide-slate-700">
                        <thead class="bg-gray-50 dark:bg-slate-900/60">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">
                                ...
                                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">P. Compra</th>
                                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">P. Venta</th>
                                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">Margen</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">Estado</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">Acción</th>
                            </tr>
                        </thead>
                                ...
                                @foreach($porCapacidadGlobal as $capacidad => $variantes)
                                    ...
                                    <tr class="hover:bg-blue-50/30 dark:hover:bg-blue-900/30 transition-colors {{ $precio ? '' : 'bg-amber-50/30' }}">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            @if($precio)
                                                @if($precio->activo)
                                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800">
                                                        <i class="fas fa-circle text-[6px]"></i> Activo
                                                    </span>
                                                @else
                                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400 border border-gray-200 dark:border-slate-700">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <div class="flex items-center justify-center gap-1.5 flex-wrap">
                                                ...
                                                @else
                                                    <span class="text-xs text-gray-400 dark:text-slate-500 italic">Registra desde el formulario</span>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            @else
                                ...
                                @if($precioBase)
                                    <tr class="hover:bg-blue-50/30 dark:hover:bg-blue-900/30 transition-colors">
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <span class="text-sm font-bold text-blue-700 dark:text-blue-300">S/ {{ number_format($precioBase->precio, 2) }}</span>
                                        </td>
                                        <td class="px-4 py-3 text-right">
                                    ...
                                    <tr>
                                        <td colspan="8" class="py-12 text-center">
                                            <div class="w-14 h-14 bg-gray-100 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-3">
                                                <i class="fas fa-globe text-2xl text-gray-400 dark:text-slate-500"></i>
                                            </div>
                                            <p class="text-gray-500 dark:text-slate-400 font-medium text-sm">Sin precio global registrado</p>
                                            <p class="text-gray-400 dark:text-slate-500 text-xs mt-1">Usa el formulario de la izquierda para registrar el primer precio.</p>
                                        </td>
                                    </tr>
                                @endif
                            @endif
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden"
    ...
    <div x-show="modal.show" x-cloak
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-4"
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="text-base font-bold text-gray-900 dark:text-slate-100" x-text="modal.title"></h3>
                    <p class="text-sm text-gray-500 dark:text-slate-400 mt-1" x-text="modal.body"></p>
                </div>
            </div>
            <div class="flex gap-2 justify-end pt-1">
                <button type="button" @click="modal.show = false"
                        class="px-4 py-2 rounded-lg text-sm font-medium bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors">
                    Cancelar
                </button>
                <button type="button"
    ...
    <div x-show="inputModal.show" x-cloak
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-4"
            ...
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-base font-bold text-gray-900 dark:text-slate-100" x-text="inputModal.label"></h3>
            </div>
            <input type="number" x-model="inputModal.value" :placeholder="inputModal.placeholder"
                   ...
                   class="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" autofocus>
    ...
    @if($preciosPorTienda->count())
        ...
        <div class="px-5 py-3 bg-gray-50 dark:bg-slate-900/60 border-b border-gray-100 dark:border-slate-700 flex flex-wrap items-center gap-2">
        ...
        {{-- Vista agrupada por tienda → capacidad --}}
                <div class="divide-y divide-gray-100 dark:divide-slate-700">
                    ...
                    @foreach($preciosPorTiendaAgrupados as $nombreTienda => $preciosTienda)
                        ...
                        <div class="store-group">
                            {{-- Header de tienda --}}
                            <div class="bg-gray-50/50 dark:bg-slate-800/50 hover:bg-gray-100/50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer"
                                ...
                                <div class="px-5 py-3 flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        <input type="checkbox"
                                            ...
                                            class="w-4 h-4 rounded border-gray-300 dark:border-slate-600 text-indigo-600 dark:text-indigo-400 focus:ring-indigo-500">
                                        ...
                                        <div>
                                            <p class="text-sm font-semibold text-gray-800 dark:text-slate-200">{{ $nombreTienda }}</p>
                                            @php
                                    ...
                                    <i class="fas fa-chevron-down text-gray-400 dark:text-slate-500 text-xs transition-transform"
                                    ...
                                    <table class="min-w-full">
                                        <thead>
                                            <tr class="border-b border-gray-100 dark:border-slate-700 bg-indigo-50/30 dark:bg-indigo-900/30">
                                                ...
                                                <th class="px-4 py-2 text-right text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide">P. Venta</th>
                                                <th class="px-4 py-2 text-right text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide">Margen</th>
                                                <th class="px-4 py-2 text-center text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide">Estado</th>
                                                <th class="px-4 py-2 text-center text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide">Acciones</th>
                                            </tr>
                                        </thead>
                                                    ...
                                                    <td class="pl-14 pr-4 py-2.5">
                                                        <div class="flex items-center gap-2">
                                                            <input type="checkbox"
                                                                ...
                                                                class="w-3.5 h-3.5 rounded border-gray-300 dark:border-slate-600 text-indigo-600 dark:text-indigo-400 focus:ring-indigo-500">
                                                            <i class="fas fa-microchip text-indigo-400 text-xs"></i>
                                                            <span class="text-sm font-medium text-gray-800 dark:text-slate-200">{{ $cap ?: 'Sin capacidad' }}</span>
                                                            @php $coloresReales = $porCapacidadGlobal->get($cap)?->count() ?? $preciosCap->count(); @endphp
                                                            <span class="text-xs text-gray-400 dark:text-slate-500">({{ $coloresReales }} {{ $coloresReales === 1 ? 'color' : 'colores' }})</span>
                                                        </div>
                                                    </td>
                                                    <td class="px-4 py-2.5 text-sm text-gray-500 dark:text-slate-400">
                                                        {{ $pRep->proveedor?->razon_social ?? '—' }}
                                                    </td>
                                                    <td class="px-4 py-2.5 text-sm text-right text-gray-600 dark:text-slate-400">
                                                        {{ $pRep->precio_compra ? 'S/ ' . number_format($pRep->precio_compra, 2) : '—' }}
                                                    </td>
                                                    <td class="px-4 py-2.5 text-right">
                                                        <span class="text-sm font-bold text-indigo-700 dark:text-indigo-300">S/ {{ number_format($pRep->precio, 2) }}</span>
                                                    </td>
                                                    <td class="px-4 py-2.5 text-right">
                                                        @if($pRep->margen !== null)
                ...
                <div class="px-5 py-3 border-t border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/60">
                    <span class="text-xs text-gray-500 dark:text-slate-400">Mostrando {{ $preciosPorTienda->count() }} registro(s)</span>
                </div>
            @else
                <div class="py-10 text-center">
                    <div class="w-14 h-14 bg-gray-100 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-3">
                        <i class="fas fa-store text-2xl text-gray-400 dark:text-slate-500"></i>
                    </div>
                    <p class="text-gray-500 dark:text-slate-400 font-medium text-sm">Sin precios por tienda</p>
