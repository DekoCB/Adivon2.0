...
@section('title', 'Dashboard Cajas')

@section('content')
<div class="min-h-screen" x-data="{ refreshIn: 30 }" x-init="setInterval(() =>
 { if(--refreshIn <= 0) location.reload(); }, 1000)">

    {{-- Header --}}
    <div class="bg-white dark:bg-slate-800 shadow-sm px-6 py-4 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-800 dark:text-slate-200">Dashboard Ejecutivo de Cajas</h1>
            <p class="text-sm text-gray-500 dark:text-slate-400">
                Supervisión en tiempo real · Actualiza en
                <span class="font-semibold text-blue-600 dark:text-blue-400" x-text="refreshIn"></span> s
    ...
    <div class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border-l-4 border-blue-500">
                <p class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Ventas hoy</p>
                <p class="text-2xl font-bold text-gray-800 dark:text-slate-200 mt-1">S/ {{ number_format($totalVentasHoy, 2) }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border-l-4 border-green-500">
                <p class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Cajas abiertas</p>
                <p class="text-2xl font-bold text-gray-800 dark:text-slate-200 mt-1">{{ $cajasAbiertas->count() }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border-l-4 border-purple-500">
                <p class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Sucursales activas</p>
                <p class="text-2xl font-bold text-gray-800 dark:text-slate-200 mt-1">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border-l-4 {{ abs($diferenciasMes) > 0 ? 'border-red-500' : 'border-gray-300' }}">
                <p class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Diferencias (mes)</p>
                <p class="text-2xl font-bold {{ $diferenciasMes < 0 ? 'text-red-600' : 'text-gray-800' }} mt-1">
                    S/ {{ number_format($diferenciasMes, 2) }}
                </p>
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ now()->format('m/Y') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                @forelse($estadoSucursales as $item)
                    ...
                    <div class="border rounded-lg p-4 {{ $c['bg'] }}">
                        <div class="flex items-start justify-between">
                            <div class="min-w-0">
                                <p class="font-semibold text-gray-800 dark:text-slate-200 text-sm truncate">{{ $item['sucursal']->nombre }}</p>
                        ...
                        @if($item['caja_abierta'])
                            <div class="mt-3 pt-3 border-t border-black/5 grid grid-cols-2 gap-2 text-xs">
                                <div>
                                    <p class="text-gray-400 dark:text-slate-500">Monto</p>
                                    <p class="font-semibold text-gray-700 dark:text-slate-300">S/ {{ number_format($item['caja_abierta']->monto_final, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-gray-400 dark:text-slate-500">Abierta hace</p>
                            ...
                            <a href="{{ route('admin.cajas.show', $item['caja_abierta']->id) }}"
                               class="text-xs text-blue-600 dark:text-blue-400 hover:underline mt-2 block">Ver caja →</a>
                ...
                @empty
                    <p class="text-sm text-gray-400 dark:text-slate-500 col-span-full">No hay sucursales registradas.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5">
                    ...
                    <div class="mb-3">
                        <div class="flex justify-between items-center mb-1">
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-bold text-gray-400 dark:text-slate-500 w-4">#{{ $i+1 }}</span>
                                <span class="text-sm text-gray-700 dark:text-slate-300">{{ $v->name }}</span>
                            </div>
                            <div class="text-right">
                                <span class="text-sm font-semibold text-gray-800 dark:text-slate-200">S/ {{ number_format($v->total, 2) }}</span>
                                <span class="text-xs text-gray-400 dark:text-slate-500 ml-1">({{ $v->ventas }} vtas)</span>
                            </div>
                        </div>
                ...
                @empty
                    <p class="text-sm text-gray-400 dark:text-slate-500 text-center py-6">Sin ventas registradas hoy.</p>
                @endforelse
            </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex justify-between text-xs mb-1">
                                    <span class="text-gray-600 dark:text-slate-400">{{ $m['label'] }}</span>
                                    <span class="font-semibold text-gray-800 dark:text-slate-200">S/ {{ number_format($monto, 2) }}</span>
                                </div>
                            ...
                            <span class="text-xs text-gray-400 dark:text-slate-500 w-10 text-right shrink-0">
