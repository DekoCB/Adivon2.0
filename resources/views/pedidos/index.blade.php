        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-blue-500 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 dark:text-slate-400">Total Pedidos</p>
                        <p class="text-3xl font-bold text-gray-800 dark:text-slate-200">{{ $stats['total'] }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-yellow-500 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-800 dark:text-slate-200">{{ $stats['pendiente'] }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-indigo-500 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 dark:text-slate-400">Aprobados</p>
                        <p class="text-3xl font-bold text-gray-800 dark:text-slate-200">{{ $stats['aprobado'] }}</p>
                    </div>
                    <div class="bg-indigo-100 dark:bg-indigo-900/40 rounded-full p-3">
                        <i class="fas fa-check text-indigo-600 dark:text-indigo-400 text-xl"></i>
                    </div>
                </div>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-green-500 p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-800 dark:text-slate-200">{{ $stats['recibido'] }}</p>
                    </div>
        ...
        <x-filter-bar action="{{ route('pedidos.index') }}" :filters="['buscar','estado','fecha_desde','fecha_hasta']">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
                <div class="lg:col-span-2">
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Buscar código / proveedor</label>
                    <div class="relative">
                ...
                <div>
                    ...
                    <select name="estado" class="w-full py-2 px-3 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <x-data-table :paginator="$pedidos">
            <x-slot:cardHeader>
                <h3 class="text-lg font-semibold text-gray-800 dark:text-slate-200">
                    <i class="fas fa-list mr-2 text-blue-600 dark:text-blue-400"></i>Lista de Pedidos
                    <span class="text-sm font-normal text-gray-500 dark:text-slate-400 ml-2">({{ $pedidos->total() }} en total)</span>
                </h3>
                ...
                <x-th>Acciones</x-th>
                        ...
                        @forelse($pedidos as $pedido)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/60 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-slate-100">{{ $pedido->proveedor->razon_social ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{{ $pedido->almacen->nombre ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{{ $pedido->fecha->format('d/m/Y') }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{{ $pedido->fecha_esperada ? $pedido->fecha_esperada->format('d/m/Y') : '-' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{{ $pedido->usuario->name ?? '-' }}</td>
                                <td class="px-6 py-4">
                                    @php
                                    ...
                                    @if($pedido->estado === 'recibido' && $pedido->compra)
                                        <a href="{{ route('compras.show', $pedido->compra) }}" class="block text-[11px] text-blue-600 dark:text-blue-400 hover:underline mt-0.5">
                                            <i class="fas fa-file-invoice mr-0.5"></i>{{ $pedido->compra->numero_factura }}
                                        </a>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <a href="{{ route('pedidos.show', $pedido) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800" title="Ver">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-gray-500 dark:text-slate-400">
                                    ...
                                    <a href="{{ route('pedidos.create') }}" class="text-blue-600 dark:text-blue-400 hover:underline mt-2 inline-block text-sm">Crear primer pedido</a>
                                </td>
                            </tr>
                        @endforelse
        </x-data-table>
