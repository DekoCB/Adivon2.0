...
<div class="flex flex-wrap gap-3 mb-6">
            <a href="{{ route('devoluciones.index') }}" class="text-sm text-gray-500 dark:text-slate-400 hover:text-blue-700 flex items-center gap-1">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-md p-6 mb-6"
            ...
            <h2 class="text-base font-bold text-gray-800 dark:text-slate-200 mb-4 flex items-center gap-2">
                ...
                Seleccionar Cliente
                @if($clienteId)
                    <span class="ml-2 text-xs font-normal text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/40 px-2 py-0.5 rounded-full">
            ...
            <div class="relative max-w-md">
                <div class="relative">
                    <span class="absolute inset-y-0 left-3 flex items-center text-gray-400 dark:text-slate-500 pointer-events-none">
                    ...
                    <input
                        ...
                        class="w-full pl-9 pr-10 py-2.5 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-slate-800"
                        autocomplete="off"
                    />
                    <button x-show="buscar" @click="buscar = ''; open = false; window.location.href = window._devCreateUrl"
                            class="absolute inset-y-0 right-3 flex items-center text-gray-400 dark:text-slate-500 hover:text-red-500">
                ...
                <div x-show="open && filtered.length > 0"
                     ...
                     class="absolute z-20 left-0 right-0 mt-1 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-lg max-h-64 overflow-y-auto"
                     style="display:none">
                    <template x-for="c in filtered" :key="c.id">
                        <button type="button"
                            ...
                            <span x-text="c.doc" class="text-xs font-mono text-gray-400 dark:text-slate-500 shrink-0"></span>
                        </button>
                    </template>
                </div>

                <div x-show="open && buscar && filtered.length === 0"
                     class="absolute z-20 left-0 right-0 mt-1 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-lg px-4 py-3 text-sm text-gray-400 dark:text-slate-500"
                     style="display:none">
        ...
        <form action="{{ route('devoluciones.store') }}" method="POST" id="form-devolucion"
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-md p-6 mb-6">
                <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                    <h2 class="text-base font-bold text-gray-800 dark:text-slate-200 flex items-center gap-2">
                    ...
                    <span class="text-sm text-gray-500 dark:text-slate-400">
                        Total a devolver: <span class="font-bold text-orange-600 dark:text-orange-400" x-text="totalUnidades"></span> unidad(es)
                    </span>
                </div>
                ...
                <div class="mb-4">
                    <div class="relative max-w-sm">
                        ...
                        <input type="text"
                               ...
                               class="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-orange-400 bg-gray-50 dark:bg-slate-900/60">
                    </div>
                </div>
                ...
                <div class="mb-4 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden"
                    ...
                    {{-- Cabecera --}}
                    <div class="bg-gray-50 dark:bg-slate-900/60 px-4 py-3 flex flex-wrap items-center justify-between gap-2">
                        <div>
                            <span class="font-semibold text-gray-800 dark:text-slate-200 text-sm">{{ $venta->codigo }}</span>
                            <span class="text-gray-400 dark:text-slate-500 text-xs ml-2">{{ $venta->fecha->format('d/m/Y') }}</span>
                        </div>
                        <div class="flex items-center gap-3 text-xs text-gray-500 dark:text-slate-400">
                            ...
                            <button type="button" @click="clearVenta('{{ $venta->id }}')"
                                    class="px-2 py-1 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-600 dark:text-slate-400 rounded font-medium transition">
                                Nada
                            </button>
                        </div>
                    </div>
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-100 dark:bg-slate-700">
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 dark:text-slate-400">Producto</th>
                                <th class="px-4 py-2 text-center text-xs font-semibold text-gray-500 dark:text-slate-400">Vendido</th>
                                <th class="px-4 py-2 text-center text-xs font-semibold text-gray-500 dark:text-slate-400">Ya devuelto</th>
                                <th class="px-4 py-2 text-center text-xs font-semibold text-gray-500 dark:text-slate-400 w-36">Cant. a devolver</th>
                                <th class="px-4 py-2 text-right text-xs font-semibold text-gray-500 dark:text-slate-400">P. unit.</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            ...
                            <tr class="{{ $disponible === 0 ? 'opacity-40 bg-gray-50' : 'hover:bg-orange-50' }} transition-colors"
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @if($detalle->variante)
                                        <span class="text-xs text-indigo-600 dark:text-indigo-400 ml-1 font-medium">{{ $detalle->variante->nombre_completo }}</span>
                                    @endif
                                    @if($detalle->imei)
                                        <span class="block text-xs font-mono text-purple-600 dark:text-purple-400 mt-0.5">
                                            <i class="fas fa-mobile-alt mr-1"></i>{{ $detalle->imei->codigo_imei }}
                                        </span>
                                    @endif
                                    @if($disponible === 0)
                                        <span class="block text-xs text-green-600 dark:text-green-400 font-medium mt-0.5">
                                ...
                                <td class="px-4 py-3 text-center">
                                    @if($yaDevuelto > 0)
                                        <span class="text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded-full">
                                            {{ $yaDevuelto }}
                                        </span>
                                    @else
                                ...
                                <td class="px-4 py-3 text-center">
                                    @if($disponible > 0)
                                        <input type="number"
                                               ...
                                               class="qty-input w-20 text-center border border-gray-300 dark:border-slate-600 rounded-lg py-1.5 px-2 text-sm font-semibold focus:ring-2 focus:ring-orange-400 focus:border-orange-400"
                                               min="0"
                                               max="{{ $disponible }}"
                                               value="{{ old("cantidades.{$detalle->id}", 0) }}"
                                               data-venta="{{ $venta->id }}">
                                        <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">máx {{ $disponible }}</p>
                                    @else
                ...
                <div x-show="filtro && document.querySelectorAll('[data-nombre]').length > 0 &&
                     ...
                     class="text-center py-8 text-gray-400 dark:text-slate-500 text-sm" style="display:none">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-md p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                    <div>
                        ...
                        <select name="almacen_id" required
                                class="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-green-500 bg-white dark:bg-slate-800">
                    ...
                    <div>
                        ...
                        <input type="text" name="observaciones" maxlength="255"
                               class="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-green-500"
                ...
                {{-- Guía de remisión --}}
                <details class="border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden" {{ old('guia.fecha_traslado') ? 'open' : '' }}>
                    <summary class="px-4 py-3 text-sm font-medium text-gray-600 dark:text-slate-400 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700/60 flex items-center gap-2 select-none">
                        <i class="fas fa-file-invoice text-emerald-500"></i>
                        Datos de Guía de Remisión
                        <span class="ml-1 text-xs text-gray-400 dark:text-slate-500 font-normal">(opcional)</span>
                        <i class="fas fa-chevron-down ml-auto text-gray-400 dark:text-slate-500 text-xs"></i>
                    </summary>
                    <div class="p-4 border-t border-gray-100 dark:border-slate-700 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-gray-50 dark:bg-slate-900/60">
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Modalidad</label>
                            <select name="guia[modalidad]"
                                    class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 focus:ring-2 focus:ring-emerald-400">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Fecha Traslado</label>
                            <input type="date" name="guia[fecha_traslado]"
                                   value="{{ old('guia.fecha_traslado') }}"
                                   class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-400">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Dirección Partida</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Dirección Llegada</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">DNI Conductor</label>
                            <input type="text" name="guia[conductor_dni]" maxlength="8"
                                   value="{{ old('guia.conductor_dni') }}"
                                   class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-400 font-mono">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Nombre Conductor</label>
                            <input type="text" name="guia[conductor_nombre]"
                                   value="{{ old('guia.conductor_nombre') }}"
                                   class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-400">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Licencia</label>
                            <input type="text" name="guia[conductor_licencia]" maxlength="20"
                                   value="{{ old('guia.conductor_licencia') }}"
                                   class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-400 font-mono">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Placa Vehículo</label>
                            <input type="text" name="guia[placa_vehiculo]" maxlength="20"
                                   value="{{ old('guia.placa_vehiculo') }}"
                                   class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-400 font-mono uppercase">
                        </div>
                    </div>
                </details>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-500 dark:text-slate-400" x-show="totalUnidades > 0">
                ...
                <div class="flex gap-3 ml-auto">
                    <a href="{{ route('devoluciones.index') }}"
                       class="bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 text-gray-700 dark:text-slate-300 font-semibold py-2.5 px-6 rounded-xl transition">
                        Cancelar
                    </a>
                    <button type="submit"
        ...
        @elseif($clienteId && $ventas->isEmpty())
            <div class="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 text-yellow-800 dark:text-yellow-300 p-5 rounded-xl flex items-center gap-3">
