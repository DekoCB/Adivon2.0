...
{{-- KPIs --}}
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-gray-500 dark:text-slate-400">Comisiones por cobrar</p>
                <p class="text-lg font-bold text-gray-800 dark:text-slate-200">S/ {{ number_format($totales['comision_pendiente'], 2) }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-gray-500 dark:text-slate-400">Comisiones cobradas</p>
                <p class="text-lg font-bold text-gray-800 dark:text-slate-200">S/ {{ number_format($totales['comision_pagado'], 2) }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-gray-500 dark:text-slate-400">Bonos por cobrar</p>
                <p class="text-lg font-bold text-gray-800 dark:text-slate-200">S/ {{ number_format($totales['bonus_pendiente'], 2) }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-gray-500 dark:text-slate-400">Total acumulado</p>
    ...
    <x-filter-bar :filters="['estado','fecha_desde','fecha_hasta']" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <select name="estado" class="border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 focus:outline-none">
                <option value="">Todos los estados</option>
                <option value="pendiente" {{ request('estado') === 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                <option value="pagado"    {{ request('estado') === 'pagado'    ? 'selected' : '' }}>Pagado</option>
            </select>
            <input type="date" name="fecha_desde" value="{{ request('fecha_desde') }}"
                   class="border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 focus:outline-none">
    ...
    <div class="flex gap-2 mb-4">
        <button @click="tab='comisiones'"
                :class="tab==='comisiones' ? 'bg-blue-600 text-white shadow' : 'bg-white text-gray-600 dark:text-slate-400 border border-gray-200 dark:border-slate-700 hover:bg-gray-50'"
        ...
        <button @click="tab='bonos'"
                :class="tab==='bonos' ? 'bg-amber-500 text-white shadow' : 'bg-white text-gray-600 dark:text-slate-400 border border-gray-200 dark:border-slate-700 hover:bg-gray-50'"
                class="px-4 py-2 rounded-xl text-sm font-medium transition-all flex items-center gap-2">
            ...
            <x-data-table :paginator="$comisiones">
                <x-slot:head>
                    ...
                    <x-th class="text-center">Estado</x-th>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/60 transition-colors">
                            ...
                            <td class="px-4 py-3 text-gray-700 dark:text-slate-300 max-w-[160px] truncate">
                                {{ $c->detalleVenta?->producto?->nombre ?? '—' }}
                            </td>
                            <td class="px-4 py-3">
                                <span class="text-xs text-gray-500 dark:text-slate-400">{{ $c->regla?->nombre ?? '—' }}</span>
                                <span class="ml-1 text-[10px] px-1.5 py-0.5 rounded-full {{ $c->tipo_calculo === 'porcentaje_margen' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500' }}">
                            ...
                            <td class="px-4 py-3 text-center">
                                @if($c->estado === 'pagado')
                                    ...
                                    @if($c->fecha_pago)
                                        <div class="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">{{ \Carbon\Carbon::parse($c->fecha_pago)->format('d/m/Y') }}</div>
                                    @endif
                                @else
                ...
                <x-slot:tfoot>
                        <tr class="bg-blue-50 dark:bg-blue-900/30 border-t border-blue-100">
                            ...
                            <td class="px-4 py-3 text-right font-bold text-blue-700 dark:text-blue-300">
            ...
            <x-data-table :paginator="$bonos">
                <x-slot:head>
                    ...
                    <x-th class="text-center">Estado</x-th>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/60 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <p class="text-gray-700 dark:text-slate-300 text-xs">{{ $b->regla?->nombre ?? '—' }}</p>
                                ...
                                @elseif($b->detalleVenta?->venta)
                                    <p class="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">Venta {{ $b->detalleVenta->venta->codigo }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-600 dark:text-slate-400 text-xs max-w-[140px] truncate">
                            ...
                            <td class="px-4 py-3 text-center">
                                @if($b->estado === 'pagado')
                                    <x-badge tone="green" icon="fa-check">Cobrado</x-badge>
                                    @if($b->fecha_pago)
                                        <div class="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">{{ \Carbon\Carbon::parse($b->fecha_pago)->format('d/m/Y') }}</div>
                                    @endif
                                @else
                                    <x-badge tone="amber" icon="fa-clock">Pendiente</x-badge>
                                @endif
                ...
                <x-slot:tfoot>
                        <tr class="bg-amber-50 dark:bg-amber-900/30 border-t border-amber-100">
                            ...
                            <td class="px-4 py-3 text-right font-bold text-amber-700 dark:text-amber-300">
    ...
    <div class="mt-6 bg-blue-50 dark:bg-blue-900/30 border border-blue-100 rounded-2xl p-4 flex gap-3">
        ...
        <div class="text-sm text-blue-700 dark:text-blue-300">
            ...
            <p class="mt-1 text-blue-600 dark:text-blue-400">Las comisiones y bonos se generan automáticamente al registrar cada venta. El pago lo realiza el administrador. Si tienes dudas sobre algún monto, consulta con tu supervisor.</p>
        </div>
    </div>
