...
<div>
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-bold text-gray-800 dark:text-slate-200">Conteos registrados</h2>
                ...
                <x-th class="text-center">Acciones</x-th>
                        ...
                        @forelse($conteos as $c)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/60">
                                <td class="px-4 py-3 font-medium text-gray-800 dark:text-slate-200">{{ $c->nombre }}</td>
                                <td class="px-4 py-3 text-gray-600 dark:text-slate-400">{{ $c->almacen->nombre }}</td>
                                <td class="px-4 py-3 text-gray-600 dark:text-slate-400">{{ $c->usuario->name }}</td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <span class="text-xs text-gray-600 dark:text-slate-400">{{ $contados }}/{{ $total }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-center">
                                ...
                                <td class="px-4 py-3 text-center text-gray-500 dark:text-slate-400 text-xs">{{ $c->created_at->format('d/m/Y') }}</td>
                                <td class="px-4 py-3 text-center">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-12 text-center text-gray-400 dark:text-slate-500">
                                    ...
                                    No hay conteos registrados. <a href="{{ route('inventario-fisico.create') }}" class="text-blue-600 dark:text-blue-400 hover:underline">Crear el primero</a>
                                </td>
                            </tr>
                        @endforelse
        </x-data-table>
