...
class CompraController extends Controller
{
    ...
    public function create()
    {
        ...
        // Si venimos de "Recibir mercadería" de un Pedido a Proveedor, precargar
        // proveedor/almacén/productos para no tener que volver a tipearlos.
        $pedidoOrigen = null;
        if (request()->filled('pedido_id')) {
            $pedido = \App\Models\Pedido::with('detalles.producto', 'detalles.variante')
                ->find(request()->integer('pedido_id'));

            if ($pedido && $pedido->estado === 'aprobado' && !$pedido->compra()->exists()) {
                $pedidoOrigen = [
                    'id'           => $pedido->id,
        ...
        return view('compras.create', compact('proveedores', 'almacenesCentral', 'almacenesTienda', 'productos', 'colores', 'marcas', 'categorias', 'unidades', 'pedidoOrigen'));
    }
    ...
    public function storeBulkImeis(Request $request, Compra $compra, DetalleCompra $detalle)
    {
        ...
        DB::transaction(function () use ($request, $compra, $detalle) {
            ...
            // Recontar desde la fuente real (imeis) en vez de incrementar: mantiene
            // producto.stock_actual / producto_variante.stock_actual exactos aunque
            // esas columnas ya estuvieran desincronizadas por otro flujo.
            $totalProducto = Imei::where('producto_id', $detalle->producto_id)
                ->where('estado_imei', 'en_stock')
                ->count();
            Producto::where('id', $detalle->producto_id)
                ->update(['stock_actual' => $totalProducto]);

            if ($detalle->variante_id) {
                $totalVariante = Imei::where('variante_id', $detalle->variante_id)
                    ->where('estado_imei', 'en_stock')
                    ->count();
                ProductoVariante::where('id', $detalle->variante_id)
...
public function importarIMEI(Request $request)
{
    ...
    {
        // Productos que suele vender este proveedor: comprados antes a él,
        // o vinculados explícitamente en su catálogo (productos_proveedor).
        $productos = Producto::where('estado', 'activo')
            ->where(function ($query) use ($proveedorId) {
                $query->whereHas('compras', function ($q) use ($proveedorId) {
                        $q->where('proveedor_id', $proveedorId);
                    })
                    ->orWhereHas('proveedores', function ($q) use ($proveedorId) {
                        $q->where('proveedores.id', $proveedorId);
                    });
            })
            ->orderBy('nombre')
            ->get();
